...
namespace Ups.Toolkit.SpreadsheetLight.Core
{
    public partial class SLDocument
    {
        ...
        /// <summary>
        ///     Get existing shared strings in a list of SharedStringItem objects. WARNING: This is only a snapshot. Any changes
        ///     made to the returned result are not used.
        /// </summary>
            ...
            if (wbp.SharedStringTablePart != null)
            {
                ...
                while (oxr.Read())
                    if (oxr.ElementType == typeof(SharedStringItem))
                        ForceSaveToSharedStringTable((SharedStringItem) oxr.LoadCurrentElement());
                oxr.Dispose();
        ...
        internal void WriteSharedStringTable()
        {
            if (wbp.SharedStringTablePart != null)
            {
                if (listSharedString.Count > countSharedString)
                {
                    ...
                    var diff = listSharedString.Count - countSharedString;
            ...
            else
            {
                if (listSharedString.Count > 0)
                {
                    ...
                    using (var ms = new MemoryStream())
                    {
                        ...
                        {
                            sw.Write("<x:sst count=\"{0}\" uniqueCount=\"{1}\" xmlns:x=\"{2}\">", listSharedString.Count,
                                dictSharedStringHash.Count, SLConstants.NamespaceX);
                            for (var i = 0; i < listSharedString.Count; ++i)
