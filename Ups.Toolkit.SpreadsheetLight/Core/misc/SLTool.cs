    ...
    internal partial class SLTool
    {
        ...
        internal static bool IsCellReference(string Input)
        {
            ...
            if (Regex.IsMatch(Input, @"^\$?[a-zA-Z]{1,3}\$?\d{1,7}$"))
            {
                ...
                Input = Input.Replace("$", "");
                if (FormatCellReferenceToRowColumnIndex(Input, out iRowIndex, out iColumnIndex))
                    result = true;
            }
        ...
        internal static string ToColumnName(int ColumnIndex)
        {
            ...
            var result = SLConstants.ErrorReference;
            ...
            else
            {
                ...
                result = string.Format("{0}{1}{2}", (char) (iLetterPos3 + 64), (char) (iLetterPos2 + 64),
                    (char) (iLetterPos1 + 64));
            }
        ...
        internal static int ToColumnIndex(string Input)
        {
            ...
            else
            {
                if (Regex.IsMatch(Input, "^[a-zA-Z]{1,3}$"))
                {
                    ...
                    // And 676 = 26 * 26
                    if (Input.Length == 3)
                        iColumnIndex = Input[0]*676 + Input[1]*26 + Input[2] - 44992;
                    else if (Input.Length == 2)
                        iColumnIndex = Input[0]*26 + Input[1] - 1728;
                    else
                        iColumnIndex = Input[0] - 64;
                }
            }
        ...
        internal static string FormatWorksheetNameForFormula(string WorksheetName)
        {
            ...
            var result = WorksheetName.Replace("'", "''");
        ...
        internal static string ToCellReference(string WorksheetName, int RowIndex, int ColumnIndex, bool IsAbsolute)
        {
            ...
            var sWorksheetName = WorksheetName;
            if (sWorksheetName.Length > 0)
                sWorksheetName = string.Format("{0}!", FormatWorksheetNameForFormula(sWorksheetName));
            else sWorksheetName = string.Empty;

            var result = string.Empty;
            if (IsAbsolute)
                result = string.Format("{0}${1}${2}", sWorksheetName, ToColumnName(iColumnIndex),
                    iRowIndex.ToString(CultureInfo.InvariantCulture));
            else
                result = string.Format("{0}{1}{2}", sWorksheetName, ToColumnName(iColumnIndex),
                    iRowIndex.ToString(CultureInfo.InvariantCulture));
        ...
        internal static string ToCellRange(string WorksheetName, int StartRowIndex, int StartColumnIndex,
        ...
        internal static string ToCellRange(string WorksheetName, int StartRowIndex, int StartColumnIndex,
            int EndRowIndex, int EndColumnIndex, bool IsAbsolute)
        {
            ...
            var result = string.Empty;
            if (IsAbsolute)
                result = string.Format("{0}${1}${2}:${3}${4}", sWorksheetName, ToColumnName(iStartColumnIndex),
                    iStartRowIndex.ToString(CultureInfo.InvariantCulture), ToColumnName(iEndColumnIndex),
                    iEndRowIndex.ToString(CultureInfo.InvariantCulture));
            else
                result = string.Format("{0}{1}{2}:{3}{4}", sWorksheetName, ToColumnName(iStartColumnIndex),
                    iStartRowIndex.ToString(CultureInfo.InvariantCulture), ToColumnName(iEndColumnIndex),
                    iEndRowIndex.ToString(CultureInfo.InvariantCulture));
        ...
        internal static bool FormatCellReferenceRangeToRowColumnIndex(string CellReferenceRange, out int StartRowIndex,
            out int StartColumnIndex, out int EndRowIndex, out int EndColumnIndex)
        {
            var result = true;
            StartRowIndex = -1;
            StartColumnIndex = -1;
            ...
            if (Regex.IsMatch(CellReferenceRange, "^[a-zA-Z]{1,3}\\d{1,7}:[a-zA-Z]{1,3}\\d{1,7}$"))
            {
                ...
                result = FormatCellReferenceToRowColumnIndex(sRef1, out StartRowIndex, out StartColumnIndex);
                result &= FormatCellReferenceToRowColumnIndex(sRef2, out EndRowIndex, out EndColumnIndex);
            }
            else
            {
                result = false;
        ...
        internal static bool FormatCellReferenceToRowColumnIndex(string CellReference, out int RowIndex,
            out int ColumnIndex)
        {
            var result = true;
            RowIndex = -1;
            ...
            if (Regex.IsMatch(CellReference, @"^[a-zA-Z]{1,3}\d{1,7}$"))
            {
                ...
                {
                    var sPrefix = CellReference.Substring(0, index);
        ...
        internal static Color ToColor(Color clr, double Tint)
        {
        ...
        {
            var s = Text.Replace(string.Format(" xmlns:x=\"{0}\"", SLConstants.NamespaceX), "");
        ...
        internal static ImageFormat TranslateImageContentType(string ContentType)
        {
            var imgtype = ImageFormat.Jpeg;
            switch (ContentType.ToLowerInvariant())
            {
                case "image/bmp":
                    imgtype = ImageFormat.Bmp;
                    break;
                case "image/gif":
                ...
                case "image/png":
                    ...
                    break;
                case "image/tiff":
                    imgtype = ImageFormat.Tiff;
                    break;
                case "image/x-icon":
                    imgtype = ImageFormat.Icon;
                    break;
                case "image/x-pcx":
                ...
                case "image/jpeg":
                    ...
                    break;
                case "image/x-emf":
                    imgtype = ImageFormat.Emf;
                    break;
                case "image/x-wmf":
                    imgtype = ImageFormat.Wmf;
                    break;
            }
        ...
        {
            var result = XmlThatsEscaped;

            try
            {
                // doesn't matter what the tag is. We just need a root XML tag.
                var sr = new StringReader(string.Format("<sl>{0}</sl>", XmlThatsEscaped));
        ...
        {
            var usablefont = new Font(FontFamily.GenericSansSerif, (float) FontSize);
            ...
            catch
            {
                ...
                var fsLastDitch = FontStyle.Regular;
        ...
        internal static string ToDotNetFormatCode(string FormatCode)
        {
            ...
            var saFormat = FormatCode.Split("\"".ToCharArray());
            for (var i = 0; i < saFormat.Length; ++i)
                if (i%2 == 0)
                {
                    ...
                    saFormat[i] = Regex.Replace(saFormat[i], "[Gg][Ee][Nn][Ee][Rr][Aa][Ll]",
                        SLConstants.GeneralFormatPlaceholder);
                    ...
                    saFormat[i] = Regex.Replace(saFormat[i], "((^|;)\\s*[^dmy]*\\s*)\\[h\\]",
                        string.Format("$1{0}", SLConstants.ElapsedHourFormatPlaceholder));
                    saFormat[i] = Regex.Replace(saFormat[i], "((^|;)\\s*[^dmy]*\\s*)\\[mm\\]",
                        string.Format("$1{0}", SLConstants.ElapsedMinuteFormatPlaceholder));
                    saFormat[i] = Regex.Replace(saFormat[i], "((^|;)\\s*[^dmy]*\\s*)\\[ss\\]",
                        string.Format("$1{0}", SLConstants.ElapsedSecondFormatPlaceholder));
        ...
        internal static string ToSampleDisplayFormat(double Data, string FormatCode)
        {
            ...
            if (bIsDate)
            {
                ...
                try
                {
                    ...
                    result = result.Replace(SLConstants.ElapsedHourFormatPlaceholder,
                        ts.TotalHours.ToString("f0", CultureInfo.InvariantCulture));
                    result = result.Replace(SLConstants.ElapsedMinuteFormatPlaceholder,
                        ts.TotalMinutes.ToString("f0", CultureInfo.InvariantCulture));
                    result = result.Replace(SLConstants.ElapsedSecondFormatPlaceholder,
                        ts.TotalSeconds.ToString("f0", CultureInfo.InvariantCulture));
                }
                catch
                {
        ...
        internal static SizeF CalculateOuterBoundsOfRotatedRectangle(float Width, float Height, double Angle)
        {
            ...
            fAngleBeta = ToRadian(fAngleBeta);

            var szf = new SizeF(0, 0);
        ...
        {
            var saText = Text.Replace("\r\n", "\n").Split('\n');
            var szfa = new SizeF[saText.Length];

            var fDoubleUnderscoreWidth = g.MeasureString("__", UsableFont).Width;

            for (var i = 0; i < saText.Length; ++i)
            {
                ...
                szfa[i].Height = (szfa[i].Height + TextRenderer.MeasureText(Text, UsableFont).Height)/2.0f;
            }

            var szf = new SizeF(0, 0);
            for (var i = 0; i < szfa.Length; ++i)
            {
        ...
        internal static SLCellPointRange TranslateReferenceToCellPointRange(string Reference)
        {
            ...
            if (index > -1)
            {
                if (FormatCellReferenceRangeToRowColumnIndex(Reference, out iStartRowIndex, out iStartColumnIndex,
            ...
            {
                if (FormatCellReferenceToRowColumnIndex(Reference, out iStartRowIndex, out iStartColumnIndex))
        ...
        {
            var pts = new List<SLCellPointRange>();
            ...
            foreach (var s in SeqRef.Items)
            {
                index = s.Value.IndexOf(":");
                if (index > -1)
                {
                    if (FormatCellReferenceRangeToRowColumnIndex(s.Value, out iStartRowIndex, out iStartColumnIndex,
                        out iEndRowIndex, out iEndColumnIndex))
                    {
                        pt = new SLCellPointRange(iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex);
                        pts.Add(pt);
                ...
                {
                    if (FormatCellReferenceToRowColumnIndex(s.Value, out iStartRowIndex, out iStartColumnIndex))
                    {
                        pt = new SLCellPointRange(iStartRowIndex, iStartColumnIndex, iStartRowIndex, iStartColumnIndex);
                        pts.Add(pt);
                    }
        ...
        {
            var list = new ListValue<StringValue>();

            var sRef = string.Empty;
            foreach (var pt in PointRange)
            {
                if ((pt.StartRowIndex == pt.EndRowIndex) && (pt.StartColumnIndex == pt.EndColumnIndex))
                    sRef = ToCellReference(pt.StartRowIndex, pt.StartColumnIndex);
                else
                    sRef = string.Format("{0}:{1}", ToCellReference(pt.StartRowIndex, pt.StartColumnIndex),
                        ToCellReference(pt.EndRowIndex, pt.EndColumnIndex));
        ...
        internal static List<SLCellPointRange> TranslateRefSeqToCellPointRange(ReferenceSequence RefSeq)
        {
            var pts = new List<SLCellPointRange>();
            ...
            var saRef = RefSeq.Text.Split(" ".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);

            foreach (var s in saRef)
            {
                index = s.IndexOf(":");
                if (index > -1)
                {
                    if (FormatCellReferenceRangeToRowColumnIndex(s, out iStartRowIndex, out iStartColumnIndex,
                        out iEndRowIndex, out iEndColumnIndex))
                    {
                        pt = new SLCellPointRange(iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex);
                        pts.Add(pt);
                ...
                {
                    if (FormatCellReferenceToRowColumnIndex(s, out iStartRowIndex, out iStartColumnIndex))
                    {
                        pt = new SLCellPointRange(iStartRowIndex, iStartColumnIndex, iStartRowIndex, iStartColumnIndex);
                        pts.Add(pt);
                    }
        ...
        internal static string TranslateCellPointRangeToReference(SLCellPointRange Range)
        {
            ...
            if ((Range.StartRowIndex == Range.EndRowIndex) && (Range.StartColumnIndex == Range.EndColumnIndex))
                result = ToCellReference(Range.StartRowIndex, Range.StartColumnIndex);
            else
                result = ToCellRange(Range.StartRowIndex, Range.StartColumnIndex, Range.EndRowIndex,
