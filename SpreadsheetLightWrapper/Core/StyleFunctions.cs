...
namespace SpreadsheetLightWrapper.Core
{
    ...
    {
        /// <summary>
        ///     Get a list of existing styles. WARNING: This is only a snapshot. Any changes made to the returned result are not
        ///     used.
        /// </summary>
        /// <returns>A list of existing SLStyle objects.</returns>
        public List<SLStyle> GetStyles()
        {
            var result = new List<SLStyle>();
            var style = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont, SimpleTheme.listThemeColors,
                SimpleTheme.listIndexedColors);
            for (var i = 0; i < listStyle.Count; ++i)
            {
        ...
        /// <summary>
        ///     Get the cell's style. The default style is returned if cell doesn't have an existing style, or if the cell
        ...
        public SLStyle GetCellStyle(string CellReference)
        {
            ...
            {
                var style = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
        ...
        /// <summary>
        ///     Get the cell's style. The default style is returned if cell doesn't have an existing style, or if the row or column
        ///     indices are invalid.
        /// </summary>
        ...
        /// <param name="Style">The style to set.</param>
        /// <returns>True if successful. False otherwise.</returns>
        public bool SetCellStyle(int StartRowIndex, int StartColumnIndex, int EndRowIndex, int EndColumnIndex,
            SLStyle Style)
        {
            ...
            {
                result = true;
                var iStyleIndex = SaveToStylesheet(Style.ToHash());
                ...
                var cellstyle = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                    SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                SLRowProperties rp;
                    ...
                    for (j = iStartColumnIndex; j <= iEndColumnIndex; ++j)
                    {
                        ...
                        if (slws.Cells.ContainsKey(pt))
                        {
                            ...
                            else
                            {
                                ...
                                c.StyleIndex = (uint) SaveToStylesheet(cellstyle.ToHash());
        ...
        /// <summary>
        ///     Apply a named cell style to a cell. Existing styles are kept, unless the chosen named cell style overrides those
        ///     styles.
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        ...
        /// <summary>
        ///     Apply a named cell style to a range of cells. Existing styles are kept, unless the chosen named cell style
        ///     overrides those styles.
        /// </summary>
        /// <param name="StartCellReference">
        ///     The cell reference of the start cell of the cell range, such as "A1". This is
        ///     typically the top-left cell.
        ...
        /// <param name="NamedCellStyle">The named cell style to be applied.</param>
        /// <returns>True if successful. False otherwise.</returns>
        public bool ApplyNamedCellStyle(string StartCellReference, string EndCellReference,
            SLNamedCellStyleValues NamedCellStyle)
        {
            var iStartRowIndex = -1;
            var iStartColumnIndex = -1;
            var iEndRowIndex = -1;
        ...
        /// <param name="NamedCellStyle">The named cell style to be applied.</param>
        /// <returns>True if successful. False otherwise.</returns>
        public bool ApplyNamedCellStyle(int StartRowIndex, int StartColumnIndex, int EndRowIndex, int EndColumnIndex,
            SLNamedCellStyleValues NamedCellStyle)
        {
            int iStartRowIndex = 1, iEndRowIndex = 1, iStartColumnIndex = 1, iEndColumnIndex = 1;
            var result = false;
        ...
        /// <summary>
        ///     Apply a named cell style to a row. Existing styles are kept, unless the chosen named cell style overrides those
        ///     styles.
        /// </summary>
        /// <param name="RowIndex">The row index.</param>
        /// <param name="NamedCellStyle">The named cell style to be applied.</param>
        ...
        /// <summary>
        ///     Apply a named cell style to a range of rows. Existing styles are kept, unless the chosen named cell style overrides
        ///     those styles.
        /// </summary>
        /// <param name="StartRowIndex">The row index of the starting row.</param>
        /// <param name="EndRowIndex">The row index of the ending row.</param>
        ...
        /// <summary>
        ///     Apply a named cell style to a column. Existing styles are kept, unless the chosen named cell style overrides those
        ///     styles.
        /// </summary>
        /// <param name="ColumnIndex">The column index.</param>
        /// <param name="NamedCellStyle">The named cell style to be applied.</param>
        ...
        /// <summary>
        ///     Apply a named cell style to a range of columns. Existing styles are kept, unless the chosen named cell style
        ///     overrides those styles.
        /// </summary>
        /// <param name="StartColumnIndex">The column index of the starting column.</param>
        /// <param name="EndColumnIndex">The column index of the ending column.</param>
        ...
        public bool ApplyNamedCellStyleToColumn(int StartColumnIndex, int EndColumnIndex,
            SLNamedCellStyleValues NamedCellStyle)
        {
        ...
        public bool SetRowStyle(int StartRowIndex, int EndRowIndex, SLStyle RowStyle)
        {
                ...
                (iEndRowIndex <= SLConstants.RowLimit))
            {
                result = true;
                var i = 0;
                var iStyleIndex = SaveToStylesheet(RowStyle.ToHash());
                SLRowProperties rp;
                for (i = iStartRowIndex; i <= iEndRowIndex; ++i)
        ...
        public bool SetColumnStyle(int StartColumnIndex, int EndColumnIndex, SLStyle ColumnStyle)
        {
                ...
                (iEndColumnIndex <= SLConstants.ColumnLimit))
            {
                result = true;
                var i = 0;
                var iStyleIndex = SaveToStylesheet(ColumnStyle.ToHash());
                SLColumnProperties cp;
                for (i = iStartColumnIndex; i <= iEndColumnIndex; ++i)
        ...
        {
            var iFromRowIndex = -1;
            var iFromColumnIndex = -1;
            var iToRowIndex = -1;
            var iToColumnIndex = -1;
            ...
            return CopyCellStyle(iFromRowIndex, iFromColumnIndex, iToRowIndex, iToColumnIndex, iToRowIndex,
                iToColumnIndex);
        }
        ...
        public bool CopyCellStyle(string FromCellReference, string ToStartCellReference, string ToEndCellReference)
        {
            var iFromRowIndex = -1;
            var iFromColumnIndex = -1;
            var iToStartRowIndex = -1;
            var iToStartColumnIndex = -1;
            var iToEndRowIndex = -1;
            var iToEndColumnIndex = -1;
            if (!SLTool.FormatCellReferenceToRowColumnIndex(FromCellReference, out iFromRowIndex, out iFromColumnIndex)
                ||
                !SLTool.FormatCellReferenceToRowColumnIndex(ToStartCellReference, out iToStartRowIndex,
                    out iToStartColumnIndex)
                ||
                !SLTool.FormatCellReferenceToRowColumnIndex(ToEndCellReference, out iToEndRowIndex,
                    out iToEndColumnIndex))
                return false;

            return CopyCellStyle(iFromRowIndex, iFromColumnIndex, iToStartRowIndex, iToStartColumnIndex, iToEndRowIndex,
                iToEndColumnIndex);
        }
        ...
        /// <param name="FromRowIndex">The row index of the cell to be copied from.</param>
        /// <param name="FromColumnIndex">The column index of the cell to be copied from.</param>
        /// <param name="ToStartRowIndex">
        ///     The row index of the starting cell of the cell range. This is typically the top-left
        ///     cell.
        /// </param>
        /// <param name="ToStartColumnIndex">
        ///     The column index of the starting cell of the cell range. This is typically the
        ///     top-left cell.
        /// </param>
        /// <param name="ToEndRowIndex">
        ///     The row index of the ending cell of the cell range. This is typically the bottom-right
        ///     cell.
        /// </param>
        /// <param name="ToEndColumnIndex">
        ///     The column index of the ending cell of the cell range. This is typically the
        ///     bottom-right cell.
        /// </param>
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCellStyle(int FromRowIndex, int FromColumnIndex, int ToStartRowIndex, int ToStartColumnIndex,
            int ToEndRowIndex, int ToEndColumnIndex)
        {
            int iStartRowIndex = 1, iEndRowIndex = 1, iStartColumnIndex = 1, iEndColumnIndex = 1;
            var result = false;
                ...
                && SLTool.CheckRowColumnIndexLimit(iEndRowIndex, iEndColumnIndex))
            {
                ...
                uint iStyleIndex = 0;
                var pt = new SLCellPoint(FromRowIndex, FromColumnIndex);
                if (slws.Cells.ContainsKey(pt))
                {
                ...
                for (var i = iStartRowIndex; i <= iEndRowIndex; ++i)
                    for (var j = iStartColumnIndex; j <= iEndColumnIndex; ++j)
                        if ((i != FromRowIndex) && (j != FromColumnIndex))
                        {
                            ...
                            else
                            {
                                iStyleIndexNew = 0;
                                if (slws.RowProperties.ContainsKey(i))
                                    iStyleIndexNew = slws.RowProperties[i].StyleIndex;
                                if ((iStyleIndexNew == 0) && slws.ColumnProperties.ContainsKey(j))
                                    iStyleIndexNew = slws.ColumnProperties[j].StyleIndex;
        ...
        public bool CopyRowStyle(int FromRowIndex, int ToStartRowIndex, int ToEndRowIndex)
        {
                ...
                && (iEndRowIndex >= 1) && (iEndRowIndex <= SLConstants.RowLimit))
            {
                ...
                var rowstyle = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                    SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
        ...
        public bool CopyColumnStyle(int FromColumnIndex, int ToStartColumnIndex, int ToEndColumnIndex)
        {
                ...
                && (iEndColumnIndex >= 1) && (iEndColumnIndex <= SLConstants.ColumnLimit))
            {
                ...
                for (i = iStartColumnIndex; i <= iEndColumnIndex; ++i)
                    if (i != FromColumnIndex)
                        ...
                        else
                        {
                            if (iStyleIndex != 0)
                            {
                                cp = new SLColumnProperties(SimpleTheme.ThemeColumnWidth,
                                    SimpleTheme.ThemeColumnWidthInEMU, SimpleTheme.ThemeMaxDigitWidth,
                                    SimpleTheme.listColumnStepSize);
                ...
                var colstyle = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                    SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
        ...
        internal void TranslateStyleIdsToStyles(ref SLStyle style)
        {
            ...
            if (style.FontId != null)
            {
                ...
                {
                    style.fontReal = new SLFont(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                        SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
        ...
        internal void TranslateStylesToStyleIds(ref SLStyle style)
        {
            ...
            if (style.nfFormatCode.FormatCode.Length > 0)
                style.NumberFormatId = (uint) SaveToStylesheetNumberingFormat(style.nfFormatCode.ToHash());
            else
                style.NumberFormatId = (uint) NumberFormatGeneralId;

            if (style.HasFont)
                style.FontId = (uint) SaveToStylesheetFont(style.fontReal.ToHash());
            else
                style.FontId = (uint) SaveToStylesheetFont(listStyleFont[0]);

            if (style.HasFill)
            {
                style.FillId = (uint) SaveToStylesheetFill(style.fillReal.ToHash());
            }
            else
            {
                var fill = new SLFill(SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                fill.SetPatternType(PatternValues.None);
                style.FillId = (uint) SaveToStylesheetFill(fill.ToHash());
            }

            if (style.HasBorder)
            {
                style.BorderId = (uint) SaveToStylesheetBorder(style.borderReal.ToHash());
            }
            else
            {
                var border = new SLBorder(SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                style.BorderId = (uint) SaveToStylesheetBorder(border.ToHash());
            }
        }
        ...
        internal void LoadStylesheet()
        {
            ...
            {
                var wbsp = wbp.WorkbookStylesPart;

                NextNumberFormatId = SLConstants.CustomNumberFormatIdStartIndex;
                StylesheetColors = null;

                using (var oxr = OpenXmlReader.Create(wbsp))
                {
                    while (oxr.Read())
                        if (oxr.ElementType == typeof(NumberingFormats))
                        {
                            NumberingFormat nf;
                            using (var oxrNF = OpenXmlReader.Create((NumberingFormats) oxr.LoadCurrentElement()))
                            {
                                while (oxrNF.Read())
                                    if (oxrNF.ElementType == typeof(NumberingFormat))
                                    {
                                        ...
                                        if ((nf.NumberFormatId != null) && (nf.FormatCode != null))
                                        {
                                            ...
                                            if (sHash.Equals(SLConstants.NumberFormatGeneral,
                                                StringComparison.OrdinalIgnoreCase))
                                            {
                                                NumberFormatGeneralText = sHash;
                                                NumberFormatGeneralId = i;
                                            }
                                            ...
                                            if (i >= NextNumberFormatId)
                                                NextNumberFormatId = NextNumberFormatId + 1;
                                        }
                                    }
                            }
                        }
                        ...
                        {
                            SLFont fontSL;
                            using (var oxrFont = OpenXmlReader.Create((Fonts) oxr.LoadCurrentElement()))
                            {
                                while (oxrFont.Read())
                                    if (oxrFont.ElementType == typeof(Font))
                                    {
                                        fontSL = new SLFont(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                                            SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                                        fontSL.FromFont((Font) oxrFont.LoadCurrentElement());
                                        ForceSaveToStylesheetFont(fontSL.ToHash());
                                    }
                            }
                        ...
                        {
                            SLFill fillSL;
                            using (var oxrFill = OpenXmlReader.Create((Fills) oxr.LoadCurrentElement()))
                            {
                                while (oxrFill.Read())
                                    if (oxrFill.ElementType == typeof(Fill))
                                    {
                                        ...
                                        ForceSaveToStylesheetFill(fillSL.ToHash());
                                    }
                            }
                        ...
                        {
                            SLBorder borderSL;
                            using (var oxrBorder = OpenXmlReader.Create((Borders) oxr.LoadCurrentElement()))
                            {
                                while (oxrBorder.Read())
                                    if (oxrBorder.ElementType == typeof(Border))
                                    {
                                        borderSL = new SLBorder(SimpleTheme.listThemeColors,
                                            SimpleTheme.listIndexedColors);
                                        borderSL.FromBorder((Border) oxrBorder.LoadCurrentElement());
                                        ForceSaveToStylesheetBorder(borderSL.ToHash());
                                    }
                            }
                        ...
                        else if (oxr.ElementType == typeof(CellStyleFormats))
                        {
                            SLStyle styleSL;
                            using (
                                var oxrCellStyleFormats =
                                    OpenXmlReader.Create((CellStyleFormats) oxr.LoadCurrentElement()))
                            {
                                while (oxrCellStyleFormats.Read())
                                    if (oxrCellStyleFormats.ElementType == typeof(CellFormat))
                                    {
                                        styleSL = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                                            SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                                        styleSL.FromCellFormat((CellFormat) oxrCellStyleFormats.LoadCurrentElement());
                                        TranslateStyleIdsToStyles(ref styleSL);
                                        ForceSaveToStylesheetCellStylesFormat(styleSL.ToHash());
                                    }
                            }
                        ...
                        {
                            SLStyle styleSL;
                            using (var oxrCellFormats = OpenXmlReader.Create((CellFormats) oxr.LoadCurrentElement()))
                            {
                                while (oxrCellFormats.Read())
                                    if (oxrCellFormats.ElementType == typeof(CellFormat))
                                    {
                                        ...
                                        TranslateStyleIdsToStyles(ref styleSL);
                                        ForceSaveToStylesheet(styleSL.ToHash());
                                    }
                            }
                        ...
                        {
                            SLCellStyle csSL;
                            using (var oxrCellStyles = OpenXmlReader.Create((CellStyles) oxr.LoadCurrentElement()))
                            {
                                while (oxrCellStyles.Read())
                                    if (oxrCellStyles.ElementType == typeof(CellStyle))
                                    {
                                        ...
                                        ForceSaveToStylesheetCellStyle(csSL.ToHash());
                                    }
                            }
                        ...
                        else if (oxr.ElementType == typeof(DifferentialFormats))
                        {
                            ...
                            using (var oxrDiff = OpenXmlReader.Create((DifferentialFormats) oxr.LoadCurrentElement()))
                            {
                                while (oxrDiff.Read())
                                    if (oxrDiff.ElementType == typeof(DifferentialFormat))
                                    {
                                        ...
                                        ForceSaveToStylesheetDifferentialFormat(dfSL.ToHash());
                                    }
                            }
                        ...
                        {
                            var tss = (TableStyles) oxr.LoadCurrentElement();
                            SLTableStyle tsSL;
                            i = 0;
                            using (var oxrTableStyles = OpenXmlReader.Create(tss))
                            {
                            ...
                            if (tss.DefaultTableStyle != null)
                                TableStylesDefaultTableStyle = tss.DefaultTableStyle.Value;
                            else
                                TableStylesDefaultTableStyle = string.Empty;

                            if (tss.DefaultPivotStyle != null)
                                TableStylesDefaultPivotStyle = tss.DefaultPivotStyle.Value;
                            else
                                TableStylesDefaultPivotStyle = string.Empty;
                        }
                        else if (oxr.ElementType == typeof(Colors))
                        {
                            StylesheetColors = (Colors) oxr.LoadCurrentElement().CloneNode(true);
                        }
                }

                // Force a "General" number format to be saved.
                // Upper case is used by LibreOffice. Is it case insensitive?
                if (NumberFormatGeneralId < 0)
                    if (!dictStyleNumberingFormat.ContainsKey(0))
                    {
                        NumberFormatGeneralId = 0;
                        NumberFormatGeneralText = SLConstants.NumberFormatGeneral;
                        dictStyleNumberingFormat[NumberFormatGeneralId] = NumberFormatGeneralText;
                        dictStyleNumberingFormatHash[NumberFormatGeneralText] = NumberFormatGeneralId;
                    }
                    else
                    {
                        NumberFormatGeneralId = NextNumberFormatId;
                        NumberFormatGeneralText = SLConstants.NumberFormatGeneral;
                        dictStyleNumberingFormat[NumberFormatGeneralId] = NumberFormatGeneralText;
                        dictStyleNumberingFormatHash[NumberFormatGeneralText] = NumberFormatGeneralId;
                ...
                {
                    var fontSL = new SLFont(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                        SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                    ...
                    SaveToStylesheetFont(fontSL.ToHash());
                }

                if (listStyleFill.Count == 0)
                {
                    var fillNone = new SLFill(SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                    fillNone.SetPatternType(PatternValues.None);
                    SaveToStylesheetFill(fillNone.ToHash());

                    var fillGray125 = new SLFill(SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                    fillGray125.SetPatternType(PatternValues.Gray125);
                    SaveToStylesheetFill(fillGray125.ToHash());
                }
                else
                {
                ...
                var borderEmpty = new SLBorder(SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                SaveToStylesheetBorder(borderEmpty.ToHash());

                var iCanonicalCellStyleFormatId = 0;
                var style = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                    SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                style.FormatCode = NumberFormatGeneralText;
            ...
            else
            {
                ...
                var fontDefault = new SLFont(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                    SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                ...
                SaveToStylesheetFont(fontDefault.ToHash());
                ...
                TableStylesDefaultTableStyle = SLConstants.DefaultTableStyle;
                TableStylesDefaultPivotStyle = SLConstants.DefaultPivotStyle;
            }
        }
        ...
        internal void WriteStylesheet()
        {
            ...
            var styleSL = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
            for (i = 0; i < listStyle.Count; ++i)
            {
                styleSL.FromHash(listStyle[i]);
                if (styleSL.nfFormatCode.FormatCode.Length > 0)
                    SaveToStylesheetNumberingFormat(styleSL.nfFormatCode.ToHash());

                if (styleSL.HasFont)
                    SaveToStylesheetFont(styleSL.fontReal.ToHash());

                if (styleSL.HasFill)
                    SaveToStylesheetFill(styleSL.fillReal.ToHash());

                if (styleSL.HasBorder)
                    SaveToStylesheetBorder(styleSL.borderReal.ToHash());
            }
            ...
            if (wbp.WorkbookStylesPart != null)
            {
                ...
                if (dictStyleNumberingFormat.Count > 0)
                    ...
                    else
                    {
                        ...
                        var listKeys = dictStyleNumberingFormat.Keys.ToList();
                        listKeys.Sort();
                        if ((listKeys[0] == 0)
                            &&
                            dictStyleNumberingFormat[listKeys[0]].Equals(SLConstants.NumberFormatGeneral,
                                StringComparison.OrdinalIgnoreCase))
                            listKeys.RemoveAt(0);
                ...
                if (listStyleBorder.Count > countStyleBorder)
                {
                    ...
                    var borderSL = new SLBorder(SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                    for (i = 0; i < diff; ++i)
                    {
                ...
                var iCanonicalCellStyleFormatId = 0;
                var styleCanonical = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                    SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                styleCanonical.FormatCode = NumberFormatGeneralText;
                ...
                iCanonicalCellStyleFormatId = SaveToStylesheetCellStylesFormat(styleCanonical.ToHash());
                TranslateStylesToStyleIds(ref styleCanonical);
                ...
                if (listStyleCellStyleFormat.Count > countStyleCellStyleFormat)
                {
                    ...
                    for (i = 0; i < diff; ++i)
                    {
                        ...
                        TranslateStylesToStyleIds(ref styleSL);
                ...
                if (listStyleDifferentialFormat.Count > countStyleDifferentialFormat)
                {
                    ...
                    for (i = 0; i < diff; ++i)
                        wbsp.Stylesheet.DifferentialFormats.Append(new DifferentialFormat
                        {
                            InnerXml = listStyleDifferentialFormat[i + countStyleDifferentialFormat]
                        });
                }
                ...
                if (listStyleTableStyle.Count > countStyleTableStyle)
                {
                    ...
                    if (TableStylesDefaultTableStyle.Length > 0)
                        wbsp.Stylesheet.TableStyles.DefaultTableStyle = TableStylesDefaultTableStyle;
                    if (TableStylesDefaultPivotStyle.Length > 0)
                        wbsp.Stylesheet.TableStyles.DefaultPivotStyle = TableStylesDefaultPivotStyle;
                    diff = listStyleTableStyle.Count - countStyleTableStyle;
                    var tsSL = new SLTableStyle();
                    for (i = 0; i < diff; ++i)
                    {
                    ...
                    using (var sw = new StreamWriter(ms))
                    {
                        ...
                        if (dictStyleNumberingFormat.Count > 0)
                            ...
                            else
                            {
                                ...
                                for (i = 0; i < listKeys.Count; ++i)
                                    sw.Write("<x:numFmt numFmtId=\"{0}\" formatCode=\"{1}\" />", listKeys[i],
                                        SLTool.XmlWrite(dictStyleNumberingFormat[listKeys[i]]));
                                sw.Write("</x:numFmts>");
                            }
                        ...
                        for (i = 0; i < listStyleBorder.Count; ++i)
                        {
                            ...
                            sw.Write("<x:border");
                            if (slb.DiagonalUp != null)
                                sw.Write(" diagonalUp=\"{0}\"", slb.DiagonalUp.Value ? "1" : "0");
                            if (slb.DiagonalDown != null)
                                sw.Write(" diagonalDown=\"{0}\"", slb.DiagonalDown.Value ? "1" : "0");
                            ...
                            if (slb.HasVerticalBorder)
                                sw.Write(SLBorderProperties.WriteToXmlTag("vertical", slb.VerticalBorder));
                            if (slb.HasHorizontalBorder)
                                sw.Write(SLBorderProperties.WriteToXmlTag("horizontal", slb.HorizontalBorder));
                        ...
                        {
                            styleSL = new SLStyle(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont, listempty,
                                listempty);
                        ...
                        for (i = 0; i < listStyle.Count; ++i)
                        {
                            ...
                            TranslateStylesToStyleIds(ref styleSL);
                            styleSL.CellStyleFormatId = (uint) iCanonicalCellStyleFormatId;
                            if (styleSL.NumberFormatId != styleCanonical.NumberFormatId)
                                styleSL.ApplyNumberFormat = true;
                            if (styleSL.FontId != styleCanonical.FontId) styleSL.ApplyFont = true;
                            if (styleSL.FillId != styleCanonical.FillId) styleSL.ApplyFill = true;
                            if (styleSL.BorderId != styleCanonical.BorderId) styleSL.ApplyBorder = true;
                            if (styleSL.HasAlignment) styleSL.ApplyAlignment = true;
                        ...
                        if (TableStylesDefaultTableStyle.Length > 0)
                            sw.Write(" defaultTableStyle=\"{0}\"", TableStylesDefaultTableStyle);
                        if (TableStylesDefaultPivotStyle.Length > 0)
                            sw.Write(" defaultPivotStyle=\"{0}\"", TableStylesDefaultPivotStyle);
                        sw.Write(">");
                        SLTableStyle tsSL;
