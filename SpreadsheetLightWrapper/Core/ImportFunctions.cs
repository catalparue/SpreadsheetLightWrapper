...
namespace SpreadsheetLightWrapper.Core
{
    ...
    {
        private List<string> GetPossibleDateFormatsForImportParsing(List<string> list1, List<string> list2,
            List<string> list3)
        {
        ...
        private void SetDateIfFailThenSetAsTextForImportParsing(string TextData, int RowIndex, int ColumnIndex,
            List<string> CustomDateFormats, CultureInfo Culture, bool HasTextQualifier, char TextQualifier,
            bool PreserveSpace)
        {
            var dtData = DateTime.Now;
            var bSuccess = false;
            if (CustomDateFormats.Count > 0)
                bSuccess = DateTime.TryParseExact(TextData, CustomDateFormats.ToArray(), Culture, DateTimeStyles.None,
                    out dtData);
            ...
            else
            {
                ...
                if (DateTime.TryParse(TextData, Culture, DateTimeStyles.None, out dtData))
                {
                    SetCellValue(RowIndex, ColumnIndex, dtData);
                }
                else
        ...
        {
            ImportText(FileName, AnchorRowIndex, AnchorColumnIndex, null);
        }
        ...
        public void ImportText(string FileName, int AnchorRowIndex, int AnchorColumnIndex, SLTextImportOptions Options)
        {
            ...
            var listYears = new List<string> {"yyyy", "yy"};
            var listMonths = new List<string> {"MMMM", "MMM", "MM", "M"};
            var listDays = new List<string> {"dd", "d"};
...
//        }
        ...
        public void ImportDataTable(int RowIndex, int ColumnIndex, DataTable Data, bool IncludeHeader)
        {
            ...
            {
                for (i = 0; i < iColumnLength; ++i)
                    SetCellValue(RowIndex, ColumnIndex + i, saColumnNames[i]);

                // get to the next row for the data part
                ++RowIndex;
            }

            var iRowCount = Data.Rows.Count;
            int iItemCount;
            int iRowIndex, iColumnIndex;
            DataRow dr;
            for (i = 0; i < iRowCount; ++i)
            {
                ...
                for (j = 0; j < iItemCount; ++j)
                {
                    ...
                    if (j <= iColumnLength)
                    {
                        ...
                        if (dr.ItemArray[j].GetType() == typeof(DBNull))
                        {
                            SetCellValue(iRowIndex, iColumnIndex, string.Empty);
                            continue;
                        }

                        if (taColumns[j] == typeof(double))
                            SetCellValue(iRowIndex, iColumnIndex, (double) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(float))
                            SetCellValue(iRowIndex, iColumnIndex, (float) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(decimal))
                            SetCellValue(iRowIndex, iColumnIndex, (decimal) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(int))
                            SetCellValue(iRowIndex, iColumnIndex, (int) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(long))
                            SetCellValue(iRowIndex, iColumnIndex, (long) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(string))
                            SetCellValue(iRowIndex, iColumnIndex, (string) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(DateTime))
                            SetCellValue(iRowIndex, iColumnIndex, (DateTime) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(short))
                            SetCellValue(iRowIndex, iColumnIndex, (short) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(ushort))
                            SetCellValue(iRowIndex, iColumnIndex, (ushort) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(uint))
                            SetCellValue(iRowIndex, iColumnIndex, (uint) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(ulong))
                            SetCellValue(iRowIndex, iColumnIndex, (ulong) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(char))
                            SetCellValue(iRowIndex, iColumnIndex, (char) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(byte))
                            SetCellValue(iRowIndex, iColumnIndex, (byte) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(sbyte))
                            SetCellValue(iRowIndex, iColumnIndex, (sbyte) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(bool))
                            SetCellValue(iRowIndex, iColumnIndex, (bool) dr.ItemArray[j]);
                        else if (taColumns[j] == typeof(TimeSpan))
                            SetCellValue(iRowIndex, iColumnIndex, ((TimeSpan) dr.ItemArray[j]).ToString());
                        // what do you do with byte[]?
                        //else if (taColumns[j] == typeof(byte[]))
                        //{
                        //}
                        else
                            SetCellValue(iRowIndex, iColumnIndex, dr.ItemArray[j].ToString());
                    }
                    else
                    {
