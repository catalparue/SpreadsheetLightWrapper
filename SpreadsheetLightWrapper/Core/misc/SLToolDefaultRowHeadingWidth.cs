...
namespace SpreadsheetLightWrapper.Core.misc
{
    internal partial class SLTool
    {
        ...
        internal static double GetDefaultRowHeadingWidth(string FontName)
        {
            ...
            var fontcheck = string.Empty;
            var fDefaultRowHeadingWidth = 20.4;
            ...
            switch (fontcheck)
            {
                ...
                default:
                    ...
                    var fFontSize = (float) Math.Round(SLConstants.DefaultFontSize*fRatio);
                    var fPadding = Math.Ceiling(5*fRatio);
                    ...
                    var sText = "300";
                    using (var bmExtra = new Bitmap(128, 128))
                    {
                        using (var g = Graphics.FromImage(bmExtra))
                        {
                            fDefaultRowHeadingWidth =
                                MeasureText(bmExtra, g, sText,
                                    GetUsableNormalFont(FontName, fFontSize, FontStyle.Regular)).Width;
                            fDefaultRowHeadingWidth += fPadding;
                            fDefaultRowHeadingWidth = fDefaultRowHeadingWidth*SLDocument.PixelToEMU/
                                                      SLConstants.PointToEMU;
                        }
                    }
                    break;
            }
