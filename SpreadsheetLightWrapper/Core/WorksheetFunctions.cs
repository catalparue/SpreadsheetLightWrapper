...
using SpreadsheetLightWrapper.Core.conditionalformatting;
using SpreadsheetLightWrapper.Core.Charts;
...
using SpreadsheetLightWrapper.Core.sparkline;
using SpreadsheetLightWrapper.Core.table;
using SpreadsheetLightWrapper.Core.workbook;
...
using ProtectedRanges = DocumentFormat.OpenXml.Spreadsheet.ProtectedRanges;
...
namespace SpreadsheetLightWrapper.Core
{
    public partial class SLDocument
    {
        ...
        ///     The name should not be blank, nor exceed 31 characters. And it cannot contain these
        ///     characters: \/?*[] It cannot be the same as an existing name (case-insensitive). But there's nothing stopping you
        ///     from using 3 spaces as a name.
        /// </param>
        ...
        public bool AddWorksheet(string WorksheetName)
        {
            ...
            slws = new SLWorksheet(SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors,
                SimpleTheme.ThemeColumnWidth, SimpleTheme.ThemeColumnWidthInEMU, SimpleTheme.ThemeMaxDigitWidth,
                SimpleTheme.listColumnStepSize, SimpleTheme.ThemeRowHeight);
            IsNewWorksheet = true;
        ...
        /// <summary>
        ///     Select an existing worksheet. If the given name doesn't match an existing worksheet, the previously selected
        ///     worksheet is used.
        /// </summary>
        /// <param name="WorksheetName">The name of an existing worksheet.</param>
        /// <returns>
        ///     True if there's an existing worksheet with that name and that worksheet is successfully selected. False
        ///     otherwise.
        /// </returns>
        public bool SelectWorksheet(string WorksheetName)
        {
            ...
            var sNewlySelectedWorksheetRelationshipID = string.Empty;
            var bFound = false;
        ...
        {
            ShowHideWorksheet(WorksheetName, SheetStateValues.Visible);
        }
        ...
        {
            ShowHideWorksheet(WorksheetName, SheetStateValues.Hidden);
        }
        ...
        {
            ShowHideWorksheet(WorksheetName, IsVeryHidden ? SheetStateValues.VeryHidden : SheetStateValues.Hidden);
        }
        ...
        /// <summary>
        ///     Indicates if the worksheet is hidden. Note that if the worksheet name isn't of an existing worksheet, the return
        ///     value is true. Think of a non-existent worksheet as very very hidden, like hidden in another dimension or
        ///     something.
        /// </summary>
        /// <param name="WorksheetName">The name of the worksheet.</param>
        ...
        /// <param name="NewWorksheetName">
        ///     The new name for the existing worksheet. The name should not be blank, nor exceed 31
        ///     characters. And it cannot contain these characters: \/?*[] It cannot be the same as an existing name
        ///     (case-insensitive).
        /// </param>
        ...
        public bool RenameWorksheet(string ExistingWorksheetName, string NewWorksheetName)
        {
            ...
            var sPattern = @"(?<pre>^|[^\]])" + SLTool.FormatWorksheetNameForFormula(ExistingWorksheetName) + "!";
            var sReplacement = "${pre}" + SLTool.FormatWorksheetNameForFormula(NewWorksheetName) + "!";
            foreach (var dn in slwb.DefinedNames)
            ...
            SLCell c;
            var listCellKeys = slws.Cells.Keys.ToList();
            ...
            foreach (var wsp in xl.WorkbookPart.WorksheetParts)
                if (wsp.DrawingsPart != null)
                {
                    ...
                    var sReplaceExisting = "(?<pre>[<:]f>)" +
                                           SLTool.FormatWorksheetNameForFormula(ExistingWorksheetName) +
                                           "(?<post>!\\$[A-Z]{1,3}\\$)";
                    var sReplaceNew = "${pre}" + SLTool.FormatWorksheetNameForFormula(NewWorksheetName) + "${post}";
                    foreach (var cp in wsp.DrawingsPart.ChartParts)
                    {
                        ...
                        if (Regex.IsMatch(chartNew.InnerXml, sReplaceExisting))
                        {
                            ...
                            cp.ChartSpace.ReplaceChild(chartNew, chartOriginal);
                            cp.ChartSpace.Save();
                        }
                    }
                }
        ...
        /// <summary>
        ///     Copy the contents of a worksheet to another worksheet. The current worksheet cannot be the source of the copy
        ///     operation, nor the destination of the copy operation.
        /// </summary>
        /// <param name="ExistingWorksheetName">The worksheet to be copied from. This cannot be the current worksheet.</param>
        /// <param name="NewWorksheetName">
        ///     The worksheet to be copied to. If this doesn't exist, a new worksheet is created. If
        ///     it's an existing worksheet, the contents of the existing worksheet will be overwritten. The new worksheet cannot be
        ...
        public bool CopyWorksheet(string ExistingWorksheetName, string NewWorksheetName)
        {
            ...
            var bExistingFound = false;
            var sExistingRelId = string.Empty;
            var bNewFound = false;
            var sNewRelId = string.Empty;
            foreach (var sheet in slwb.Sheets)
            ...
            var wspExisting = (WorksheetPart) wbp.GetPartById(sExistingRelId);
            var wspNew = wbp.AddNewPart<WorksheetPart>();

            using (var sr = new StreamReader(wspExisting.GetStream()))
            {
                using (var sw = new StreamWriter(wspNew.GetStream(FileMode.Create)))
                {
            ...
            if (wspExisting.ControlPropertiesParts != null)
            {
                ...
                foreach (var cpp in wspExisting.ControlPropertiesParts)
                {
                    cppNew = wspNew.AddNewPart<ControlPropertiesPart>(wspExisting.GetIdOfPart(cpp));
                    using (var sr = new StreamReader(cpp.GetStream()))
                    {
                        using (var sw = new StreamWriter(cppNew.GetStream(FileMode.Create)))
                        {
                            sw.Write(sr.ReadToEnd());
                        }
                    }
            ...
            if (wspExisting.CustomPropertyParts != null)
            {
                ...
                foreach (var cpp in wspExisting.CustomPropertyParts)
                {
            ...
            {
                var dp = wspExisting.DrawingsPart;
                var dpNew = wspNew.AddNewPart<DrawingsPart>(wspExisting.GetIdOfPart(dp));
                using (var sr = new StreamReader(dp.GetStream()))
                {
                    using (var sw = new StreamWriter(dpNew.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
                ...
                if (dp.CustomXmlParts != null)
                {
                    ...
                    foreach (var cxp in dp.CustomXmlParts)
                    {
                        cxpNew = dpNew.AddCustomXmlPart(cxp.ContentType, dp.GetIdOfPart(cxp));
                        FeedDataCustomXmlPart(cxpNew, cxp);
                    }
                }
                ...
                if (dp.DiagramColorsParts != null)
                {
                    ...
                    foreach (var dcp in dp.DiagramColorsParts)
                    {
                        dcpNew = dpNew.AddNewPart<DiagramColorsPart>(dp.GetIdOfPart(dcp));
                        FeedDataDiagramColorsPart(dcpNew, dcp);
                    }
                }
                ...
                if (dp.DiagramDataParts != null)
                {
                    ...
                    foreach (var ddp in dp.DiagramDataParts)
                    {
                        ddpNew = dpNew.AddNewPart<DiagramDataPart>(dp.GetIdOfPart(ddp));
                        FeedDataDiagramDataPart(ddpNew, ddp);
                    }
                }
                ...
                if (dp.DiagramLayoutDefinitionParts != null)
                {
                    ...
                    foreach (var dldp in dp.DiagramLayoutDefinitionParts)
                    {
                        dldpNew = dpNew.AddNewPart<DiagramLayoutDefinitionPart>(dp.GetIdOfPart(dldp));
                        FeedDataDiagramLayoutDefinitionPart(dldpNew, dldp);
                    }
                }
                ...
                if (dp.DiagramPersistLayoutParts != null)
                {
                    ...
                    foreach (var dplp in dp.DiagramPersistLayoutParts)
                    {
                        dplpNew = dpNew.AddNewPart<DiagramPersistLayoutPart>(dp.GetIdOfPart(dplp));
                        FeedDataDiagramPersistLayoutPart(dplpNew, dplp);
                    }
                }
                ...
                if (dp.DiagramStyleParts != null)
                {
                    ...
                    foreach (var dsp in dp.DiagramStyleParts)
                    {
                        dspNew = dpNew.AddNewPart<DiagramStylePart>(dp.GetIdOfPart(dsp));
                        FeedDataDiagramStylePart(dspNew, dsp);
                    }
                }
                ...
                foreach (var imgp in dp.ImageParts)
                {
                    imgpNew = dpNew.AddImagePart(imgp.ContentType, dp.GetIdOfPart(imgp));
                    FeedDataImagePart(imgpNew, imgp);
                }
            ...
            if (wspExisting.EmbeddedControlPersistenceBinaryDataParts != null)
            {
                ...
                foreach (var bin in wspExisting.EmbeddedControlPersistenceBinaryDataParts)
                {
                    binNew = wspNew.AddEmbeddedControlPersistenceBinaryDataPart(bin.ContentType,
                        wspExisting.GetIdOfPart(bin));
                    FeedDataEmbeddedControlPersistenceBinaryDataPart(binNew, bin);
                }
            }
            ...
            if (wspExisting.EmbeddedControlPersistenceParts != null)
            {
                ...
                foreach (var ecpp in wspExisting.EmbeddedControlPersistenceParts)
                {
                    ecppNew = wspNew.AddEmbeddedControlPersistencePart(ecpp.ContentType, wspExisting.GetIdOfPart(ecpp));
                    FeedDataEmbeddedControlPersistencePart(ecppNew, ecpp);
                }
            }
            ...
            if (wspExisting.EmbeddedObjectParts != null)
            {
                ...
                foreach (var eop in wspExisting.EmbeddedObjectParts)
                {
                    eopNew = wspNew.AddEmbeddedObjectPart(eop.ContentType);
                    FeedDataEmbeddedObjectPart(eopNew, eop);
                }
            }
            ...
            if (wspExisting.EmbeddedPackageParts != null)
            {
                ...
                foreach (var epp in wspExisting.EmbeddedPackageParts)
                {
                    eppNew = wspNew.AddEmbeddedPackagePart(epp.ContentType);
                    FeedDataEmbeddedPackagePart(eppNew, epp);
                }
            }
            ...
            if (wspExisting.HyperlinkRelationships.Count() > 0)
                foreach (var hlrel in wspExisting.HyperlinkRelationships)
            ...
            if (wspExisting.ImageParts != null)
                foreach (var imgp in wspExisting.ImageParts)
                {
            ...
            if (wspExisting.PivotTableParts != null)
            {
                ...
                foreach (var ptp in wspExisting.PivotTableParts)
                {
                    ptpNew = wspNew.AddNewPart<PivotTablePart>(wspExisting.GetIdOfPart(ptp));
                    using (var sr = new StreamReader(ptp.GetStream()))
                    {
                        using (var sw = new StreamWriter(ptpNew.GetStream(FileMode.Create)))
                        {
                            sw.Write(sr.ReadToEnd());
                        }
                    }
                    ...
                    {
                        ptpNew.AddNewPart<PivotTableCacheDefinitionPart>(
                            ptp.GetIdOfPart(ptp.PivotTableCacheDefinitionPart));
                        using (var sr = new StreamReader(ptp.PivotTableCacheDefinitionPart.GetStream()))
                        {
                            using (
                                var sw =
                                    new StreamWriter(ptpNew.PivotTableCacheDefinitionPart.GetStream(FileMode.Create)))
                            {
                                sw.Write(sr.ReadToEnd());
                            }
                        }
            ...
            if (wspExisting.QueryTableParts != null)
            {
                ...
                foreach (var qtp in wspExisting.QueryTableParts)
                {
                    qtpNew = wspNew.AddNewPart<QueryTablePart>(wspExisting.GetIdOfPart(qtp));
                    using (var sr = new StreamReader(qtp.GetStream()))
                    {
                        using (var sw = new StreamWriter(qtpNew.GetStream(FileMode.Create)))
                        {
                            sw.Write(sr.ReadToEnd());
                        }
                    }
            ...
            if (wspExisting.SingleCellTablePart != null)
            {
                ...
                using (var sr = new StreamReader(wspExisting.SingleCellTablePart.GetStream()))
                {
                    using (var sw = new StreamWriter(wspNew.SingleCellTablePart.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
            ...
            {
                SlicersPart spNew;
                foreach (var sp in wspExisting.SlicersParts)
                {
                    spNew = wspNew.AddNewPart<SlicersPart>(wspExisting.GetIdOfPart(sp));
                    using (var sr = new StreamReader(sp.GetStream()))
                    {
                        using (var sw = new StreamWriter(spNew.GetStream(FileMode.Create)))
                        {
                            sw.Write(sr.ReadToEnd());
                        }
                    }
            ...
            if (wspExisting.TableDefinitionParts != null)
            {
                ...
                QueryTablePart qtpNew;
                foreach (var tdp in wspExisting.TableDefinitionParts)
                {
                    tdpNew = wspNew.AddNewPart<TableDefinitionPart>(wspExisting.GetIdOfPart(tdp));
                    using (var sr = new StreamReader(tdp.GetStream()))
                    {
                        using (var sw = new StreamWriter(tdpNew.GetStream(FileMode.Create)))
                        {
                            sw.Write(sr.ReadToEnd());
                        }
                    }
                    ...
                    foreach (var qtp in tdp.QueryTableParts)
                    {
            ...
            if (wspExisting.VmlDrawingParts != null)
            {
                ...
                foreach (var vdp in wspExisting.VmlDrawingParts)
                {
                    vdpNew = wspNew.AddNewPart<VmlDrawingPart>(wspExisting.GetIdOfPart(vdp));
                    FeedDataVmlDrawingPart(vdpNew, vdp);
                }
            }
            ...
            if (wspExisting.WorksheetCommentsPart != null)
            {
                ...
                using (var sr = new StreamReader(wspExisting.WorksheetCommentsPart.GetStream()))
                {
                    using (var sw = new StreamWriter(wspNew.WorksheetCommentsPart.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
            ...
            if (wspExisting.WorksheetSortMapPart != null)
            {
                ...
                using (var sr = new StreamReader(wspExisting.WorksheetSortMapPart.GetStream()))
                {
                    using (var sw = new StreamWriter(wspNew.WorksheetSortMapPart.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
            ...
            {
                ++giWorksheetIdCounter;
                slwb.Sheets.Add(new SLSheet(NewWorksheetName, (uint) giWorksheetIdCounter, wbp.GetIdOfPart(wspNew),
                    SLSheetType.Worksheet));
            }
            else
            {
        ...
        /// <summary>
        ///     Get a list of names of existing worksheets currently in the spreadsheet, excluding chart sheets, macro sheets and
        ///     dialog sheets.
        /// </summary>
        /// <returns>A list of names of existing worksheets.</returns>
        public List<string> GetSheetNames()
        {
            return GetSheetNames(false);
        }
        ...
        ///     True to include chart sheets, macro sheets and dialog sheets. False to limit to only
        ///     worksheets.
        /// </param>
        ...
        /// <summary>
        ///     Get statistical information on the currently selected worksheet. NOTE: The information is only current at point of
        ///     retrieval.
        /// </summary>
        /// <returns>An SLWorksheetStatistics object with the information.</returns>
        public SLWorksheetStatistics GetWorksheetStatistics()
        {
            var wsstats = new SLWorksheetStatistics();

            var listCellRefKeys = slws.Cells.Keys.ToList();
            listCellRefKeys.Sort(new SLCellReferencePointComparer());

            List<int> intlist;

            var hsRows = new HashSet<int>(listCellRefKeys.GroupBy(g => g.RowIndex).Select(s => s.Key).ToList());
            hsRows.UnionWith(slws.RowProperties.Keys.ToList());

            if (hsRows.Count > 0)
            {
                intlist = hsRows.ToList();
                intlist.Sort();
                wsstats.iStartRowIndex = intlist[0];
                wsstats.iEndRowIndex = intlist[intlist.Count - 1];
            }

            var hsColumns = new HashSet<int>(listCellRefKeys.GroupBy(g => g.ColumnIndex).Select(s => s.Key).ToList());
            hsColumns.UnionWith(slws.ColumnProperties.Keys.ToList());

            if (hsColumns.Count > 0)
            {
                intlist = hsColumns.ToList();
                intlist.Sort();
            ...
            SLCell c;
            foreach (var pt in listCellRefKeys)
            {
                c = slws.Cells[pt];
        ...
        {
            var imgtype = SLTool.TranslateImageContentType(ExistingPart.ContentType);
            using (var bm = new Bitmap(ExistingPart.GetStream()))
            {
        ...
        {
            using (var sr = new StreamReader(ExistingPart.GetStream()))
            {
                using (var sw = new StreamWriter(NewPart.GetStream(FileMode.Create)))
                {
                    sw.Write(sr.ReadToEnd());
                }
            }
            ...
            {
                ChartPart cpNew;
                foreach (var cp in ExistingPart.ChartParts)
                {
            ...
            {
                CustomXmlPart cxpNew;
                foreach (var cxp in ExistingPart.CustomXmlParts)
                {
            ...
            {
                DiagramColorsPart dcpNew;
                foreach (var dcp in ExistingPart.DiagramColorsParts)
                {
            ...
            {
                DiagramDataPart ddpNew;
                foreach (var ddp in ExistingPart.DiagramDataParts)
                {
            ...
            {
                DiagramLayoutDefinitionPart dldpNew;
                foreach (var dldp in ExistingPart.DiagramLayoutDefinitionParts)
                {
            ...
            {
                DiagramPersistLayoutPart dplpNew;
                foreach (var dplp in ExistingPart.DiagramPersistLayoutParts)
                {
            ...
            {
                DiagramStylePart dspNew;
                foreach (var dsp in ExistingPart.DiagramStyleParts)
                {
            ...
            {
                EmbeddedControlPersistenceBinaryDataPart binNew;
                foreach (var bin in ExistingPart.EmbeddedControlPersistenceBinaryDataParts)
                {
                    binNew = NewPart.AddEmbeddedControlPersistenceBinaryDataPart(bin.ContentType,
                        ExistingPart.GetIdOfPart(bin));
                    FeedDataEmbeddedControlPersistenceBinaryDataPart(binNew, bin);
                }
            }
            ...
            {
                EmbeddedControlPersistencePart ecppNew;
                foreach (var ecpp in ExistingPart.EmbeddedControlPersistenceParts)
                {
            ...
            {
                EmbeddedObjectPart eopNew;
                foreach (var eop in ExistingPart.EmbeddedObjectParts)
                {
            ...
            {
                EmbeddedPackagePart eppNew;
                foreach (var epp in ExistingPart.EmbeddedPackageParts)
                {
            ...
            foreach (var imgp in ExistingPart.ImageParts)
            {
            ...
            if (ExistingPart.SlideCommentsPart != null)
            {
                ...
                using (var sr = new StreamReader(ExistingPart.SlideCommentsPart.GetStream()))
                {
                    using (var sw = new StreamWriter(NewPart.SlideCommentsPart.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
            ...
            {
                SlidePart spNew;
                foreach (var sp in ExistingPart.SlideParts)
                {
                    spNew = NewPart.AddNewPart<SlidePart>(ExistingPart.GetIdOfPart(sp));
                    FeedDataSlidePart(spNew, sp);
                }
            }
            ...
            if (ExistingPart.SlideSyncDataPart != null)
            {
                ...
                using (var sr = new StreamReader(ExistingPart.SlideSyncDataPart.GetStream()))
                {
                    using (var sw = new StreamWriter(NewPart.SlideSyncDataPart.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
            ...
            if (ExistingPart.ThemeOverridePart != null)
            {
                ...
                FeedDataThemeOverridePart(NewPart.ThemeOverridePart, ExistingPart.ThemeOverridePart);
            }

            if (ExistingPart.UserDefinedTagsParts != null)
            {
                UserDefinedTagsPart udtpNew;
                foreach (var udtp in ExistingPart.UserDefinedTagsParts)
                {
                    udtpNew = NewPart.AddNewPart<UserDefinedTagsPart>(ExistingPart.GetIdOfPart(udtp));
                    using (var sr = new StreamReader(udtp.GetStream()))
                    {
                        using (var sw = new StreamWriter(udtpNew.GetStream(FileMode.Create)))
                        {
                            sw.Write(sr.ReadToEnd());
                        }
                    }
            ...
            {
                VmlDrawingPart vdpNew;
                foreach (var vdp in ExistingPart.VmlDrawingParts)
                {
        ...
        internal void FeedDataChartPart(ChartPart NewPart, ChartPart ExistingPart)
        {
            ...
            if (ExistingPart.ChartDrawingPart != null)
            {
                ...
                using (var sr = new StreamReader(ExistingPart.ChartDrawingPart.GetStream()))
                {
                    using (var sw = new StreamWriter(NewPart.ChartDrawingPart.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
                ...
                {
                    NewPart.ChartDrawingPart.AddNewPart<ChartPart>(
                        ExistingPart.ChartDrawingPart.GetIdOfPart(ExistingPart.ChartDrawingPart.ChartPart));
                    FeedDataChartPart(NewPart.ChartDrawingPart.ChartPart, ExistingPart.ChartDrawingPart.ChartPart);
                }

                foreach (var imgp in ExistingPart.ChartDrawingPart.ImageParts)
                {
                    imgpNew = NewPart.ChartDrawingPart.AddImagePart(imgp.ContentType,
                        ExistingPart.ChartDrawingPart.GetIdOfPart(imgp));
                    FeedDataImagePart(imgpNew, imgp);
                }
            }
            ...
            if (ExistingPart.EmbeddedPackagePart != null)
            {
                ...
                FeedDataEmbeddedPackagePart(NewPart.EmbeddedPackagePart, ExistingPart.EmbeddedPackagePart);
            }
        ...
        internal void FeedDataCustomXmlPart(CustomXmlPart NewPart, CustomXmlPart ExistingPart)
        {
            ...
            {
                NewPart.AddNewPart<CustomXmlPropertiesPart>(
                    ExistingPart.GetIdOfPart(ExistingPart.CustomXmlPropertiesPart));
                using (var sr = new StreamReader(ExistingPart.CustomXmlPropertiesPart.GetStream()))
                {
                    using (var sw = new StreamWriter(NewPart.CustomXmlPropertiesPart.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
        ...
        internal void FeedDataDiagramDataPart(DiagramDataPart NewPart, DiagramDataPart ExistingPart)
        {
            ...
            if (ExistingPart.WorksheetParts != null)
            {
                ...
                foreach (var wsp in ExistingPart.WorksheetParts)
                {
                    ...
                    using (var sr = new StreamReader(wsp.GetStream()))
                    {
                        using (var sw = new StreamWriter(wspNew.GetStream(FileMode.Create)))
                        {
                            sw.Write(sr.ReadToEnd());
        ...
        internal void FeedDataDiagramLayoutDefinitionPart(DiagramLayoutDefinitionPart NewPart,
            DiagramLayoutDefinitionPart ExistingPart)
        {
            using (var sr = new StreamReader(ExistingPart.GetStream()))
            {
                using (var sw = new StreamWriter(NewPart.GetStream(FileMode.Create)))
        ...
        internal void FeedDataDiagramPersistLayoutPart(DiagramPersistLayoutPart NewPart,
        ...
        internal void FeedDataEmbeddedControlPersistencePart(EmbeddedControlPersistencePart NewPart,
            EmbeddedControlPersistencePart ExistingPart)
        {
            using (var sr = new StreamReader(ExistingPart.GetStream()))
            {
                using (var sw = new StreamWriter(NewPart.GetStream(FileMode.Create)))
        ...
        internal void FeedDataEmbeddedControlPersistenceBinaryDataPart(EmbeddedControlPersistenceBinaryDataPart NewPart,
            EmbeddedControlPersistenceBinaryDataPart ExistingPart)
        {
            using (var sr = new StreamReader(ExistingPart.GetStream()))
            {
                using (var sw = new StreamWriter(NewPart.GetStream(FileMode.Create)))
        ...
        internal void FeedDataVmlDrawingPart(VmlDrawingPart NewPart, VmlDrawingPart ExistingPart)
        {
            ...
            foreach (var ldtp in ExistingPart.LegacyDiagramTextParts)
            {
                ...
                using (var sr = new StreamReader(ldtp.GetStream()))
                {
                    using (var sw = new StreamWriter(ldtpNew.GetStream(FileMode.Create)))
                    {
                        sw.Write(sr.ReadToEnd());
                    }
                }
        ...
        /// <returns>
        ///     True if an actual move was done (this excludes when the given worksheet is already in the given position).
        ///     False otherwise.
        /// </returns>
        public bool MoveWorksheet(string WorksheetName, int Position)
        {
            ...
            // orig order, SheetId
            var OrigOrder = new Dictionary<uint, uint>();
            // SheetId, new order
            var NewOrder = new Dictionary<uint, uint>();

            var bFound = false;
            var sheet = slwb.Sheets[0];
        ...
        /// <summary>
        ///     Freeze panes in the worksheet (for the first workbook view). Will do nothing if both parameters are zero (because
        ///     there's nothing to freeze). Will also do nothing if either of the parameters is equal to their respective limits
        ///     (maximum number of rows, or maximum number of columns).
        /// </summary>
        ...
        public void FreezePanes(int NumberOfTopMostRows, int NumberOfLeftMostColumns)
        {
            ...
            if ((NumberOfTopMostRows >= SLConstants.RowLimit) || (NumberOfLeftMostColumns >= SLConstants.ColumnLimit))
                return;

            var slsv = new SLSheetView();
            ...
            var iRowIndex = NumberOfTopMostRows + 1;
            var iColumnIndex = NumberOfLeftMostColumns + 1;
            ...
            else
            {
                ...
                else
                {
                    ...
                    sel.SequenceOfReferences.Add(new SLCellPointRange(slws.ActiveCell.RowIndex,
                        slws.ActiveCell.ColumnIndex, slws.ActiveCell.RowIndex, slws.ActiveCell.ColumnIndex));
                }
                slsv.Selections.Add(sel);
            }

            var bFound = false;
            foreach (var sv in slws.SheetViews)
                if (sv.WorkbookViewId == 0)
                {
                    ...
                    foreach (var slsel in slsv.Selections)
        ...
        {
            UnfreezeUnsplitPanes(true);
        }

        /// <summary>
        ///     Split panes in the worksheet (for the first workbook view). Will do nothing if both number of rows and number of
        ///     columns are zero (because there's nothing to split). Will also do nothing if either is equal to their respective
        ///     limits (maximum number of rows, or maximum number of columns).
        /// </summary>
        ...
        public void SplitPanes(int NumberOfRows, int NumberOfColumns, bool ShowRowColumnHeadings)
        {
            if (!ShowRowColumnHeadings)
                SplitPanes(NumberOfRows, NumberOfColumns, false, 0, 0, false);
            else
                SplitPanes(NumberOfRows, NumberOfColumns, true, slws.SheetFormatProperties.DefaultRowHeight,
                    SLTool.GetDefaultRowHeadingWidth(SimpleTheme.MinorLatinFont), false);
        }
        ...
        /// <param name="NumberOfColumns">Number of left-most columns left of the vertical split line.</param>
        /// <param name="ShowRowColumnHeadings">True if the row and column headings are shown. False otherwise.</param>
        /// <param name="VerticalOffsetInPoints">
        ///     This is more useful when row and column headings are shown. This will be the
        ///     height of the column heading in points.
        /// </param>
        /// <param name="HorizontalOffsetInPoints">
        ///     This is more useful when row and column headings are shown. This will be the
        ///     width of the row heading in points.
        /// </param>
        public void SplitPanes(int NumberOfRows, int NumberOfColumns, bool ShowRowColumnHeadings,
            double VerticalOffsetInPoints, double HorizontalOffsetInPoints)
        {
            SplitPanes(NumberOfRows, NumberOfColumns, ShowRowColumnHeadings, VerticalOffsetInPoints,
                HorizontalOffsetInPoints, true);
        }
        ...
        ///     columns are zero (because there's nothing to split). Will also do nothing if either is equal to their respective
        ///     limits (maximum number of rows, or maximum number of columns).
        ///     The underlying engine tries to guess the individual row heights and column widths. Then the horizontal and vertical
        ///     split lines are placed based on the guesses.
        ...
        ///     width of the row heading in points.
        /// </param>
        /// <param name="ForceCustomRowColumnDimensions">
        ///     Set true to force the worksheet's row height and column width to fit the
        ///     given horizontal and vertical splits. False otherwise.
        /// </param>
        public void SplitPanes(int NumberOfRows, int NumberOfColumns, bool ShowRowColumnHeadings,
            double VerticalOffsetInPoints, double HorizontalOffsetInPoints, bool ForceCustomRowColumnDimensions)
        {
            ...
            if (ShowRowColumnHeadings)
                lWidth = (long) Math.Round(20.0*(lWidth/(double) SLConstants.PointToEMU + HorizontalOffsetInPoints));
            else
                lWidth = (long) Math.Round(20.0*(lWidth/(double) SLConstants.PointToEMU));
            ...
            // split lengths are in twentieth's of a point
            if (ShowRowColumnHeadings)
                lHeight = (long) Math.Round(20.0*(lHeight/(double) SLConstants.PointToEMU + VerticalOffsetInPoints));
            else
                lHeight = (long) Math.Round(20.0*(lHeight/(double) SLConstants.PointToEMU));
            slsv.Pane.VerticalSplit = lHeight;

            var iRowIndex = NumberOfRows + 1;
            var iColumnIndex = NumberOfColumns + 1;
        ...
        public void AddBackgroundPicture(string FileName)
        {
            ...
            slws.BackgroundPictureImagePartType = SLDrawingTool.GetImagePartType(FileName);
        }
        ...
        /// <summary>
        ///     Insert a sparkline group into the currently selected worksheet. If unsuccessful, please check that your sparkline
        ///     location is correctly set. See SetLocation() for details.
        /// </summary>
        ...
        /// <returns>True if successful. False otherwise.</returns>
        public bool InsertChart(SLChart Chart)
        {
        ...
        /// </summary>
        /// <param name="Chart">An SLChart object with the chart's properties already set.</param>
        /// <param name="ChartsheetName">
        ///     The name should not be blank, nor exceed 31 characters. And it cannot contain these
        ///     characters: \/?*[] It cannot be the same as an existing name (case-insensitive). But there's nothing stopping you
        ///     from using 3 spaces as a name.
        /// </param>
        /// <returns>True if successful. False otherwise.</returns>
        public bool InsertChart(SLChart Chart, string ChartsheetName)
        {
            ...
            var csp = wbp.AddNewPart<ChartsheetPart>();

            #region the drawing part

            var dp = csp.AddNewPart<DrawingsPart>();
            dp.WorksheetDrawing = new Xdr.WorksheetDrawing();

            var chartp = dp.AddNewPart<ChartPart>();
            ...
            gf.NonVisualGraphicFrameProperties.NonVisualGraphicFrameDrawingProperties = new Xdr.
            ...
            ++giWorksheetIdCounter;
            slwb.Sheets.Add(new SLSheet(ChartsheetName, (uint) giWorksheetIdCounter, wbp.GetIdOfPart(csp),
                SLSheetType.Chartsheet));
        ...
        private void WriteImageParts(DrawingsPart dp)
        {
            ...
            {
                var FromAnchorRowIndex = 0;
                long FromAnchorRowOffset = 0;
                var FromAnchorColumnIndex = 0;
                long FromAnchorColumnOffset = 0;
                var ToAnchorRowIndex = 4;
                long ToAnchorRowOffset = 0;
                var ToAnchorColumnIndex = 4;
                ...
                foreach (var Chart in slws.Charts)
                {
                    ...
                    gf.NonVisualGraphicFrameProperties.NonVisualGraphicFrameDrawingProperties =
                    ...
                    var tcanchor = new Xdr.TwoCellAnchor();
                    ...
                    tcanchor.FromMarker.RowOffset =
                        new Xdr.RowOffset(FromAnchorRowOffset.ToString(CultureInfo.InvariantCulture));
                    tcanchor.FromMarker.ColumnId =
                        new Xdr.ColumnId(FromAnchorColumnIndex.ToString(CultureInfo.InvariantCulture));
                    tcanchor.FromMarker.ColumnOffset =
                        new Xdr.ColumnOffset(FromAnchorColumnOffset.ToString(CultureInfo.InvariantCulture));
                    ...
                    tcanchor.ToMarker.RowOffset =
                        new Xdr.RowOffset(ToAnchorRowOffset.ToString(CultureInfo.InvariantCulture));
                    tcanchor.ToMarker.ColumnId =
                        new Xdr.ColumnId(ToAnchorColumnIndex.ToString(CultureInfo.InvariantCulture));
                    tcanchor.ToMarker.ColumnOffset =
                        new Xdr.ColumnOffset(ToAnchorColumnOffset.ToString(CultureInfo.InvariantCulture));
            ...
            if (slws.Pictures.Count > 0)
                foreach (var Picture in slws.Pictures)
                {
                    imgp = dp.AddImagePart(Picture.PictureImagePartType);

                    if (Picture.DataIsInFile)
                        using (var fs = new FileStream(Picture.PictureFileName, FileMode.Open))
                        {
                            imgp.FeedData(fs);
                        }
                    else
                        using (var ms = new MemoryStream(Picture.PictureByteData))
                        {
                            imgp.FeedData(ms);
                        }
                    ...
                    pic.NonVisualPictureProperties.NonVisualDrawingProperties.Name = string.Format("Picture {0}",
                        dp.ImageParts.Count());
                    ...
                    {
                        var hlinkrel =
                            dp.AddHyperlinkRelationship(new Uri(Picture.HyperlinkUri, Picture.HyperlinkUriKind),
                                Picture.IsHyperlinkExternal);
                        pic.NonVisualPictureProperties.NonVisualDrawingProperties.HyperlinkOnClick =
                            new A.HyperlinkOnClick {Id = hlinkrel.Id};
                    }

                    pic.NonVisualPictureProperties.NonVisualPictureDrawingProperties =
                    ...
                    {
                        var lumeffect = new A.LuminanceEffect();
                    ...
                    if (Picture.UseRelativePositioning)
                    {
                        var ocanchor = new Xdr.OneCellAnchor();
                        ...
                        ocanchor.FromMarker.ColumnId = new Xdr.ColumnId
                        {
                            Text = iColumnId.ToString(CultureInfo.InvariantCulture)
                        };
                        ocanchor.FromMarker.ColumnOffset = new Xdr.ColumnOffset
                        {
                        ...
                        ocanchor.FromMarker.RowOffset = new Xdr.RowOffset
                        {
        ...
        /// </summary>
        /// <param name="ConditionalFormatting">
        ///     An SLConditionalFormatting object with the formatting rules already set. Remember
        ///     to set at least one formatting rule (a data bar, color scale, icon set or some custom rule).
        /// </param>
        ...
        public bool AddConditionalFormatting(SLConditionalFormatting ConditionalFormatting)
        {
            ...
            {
                result = true;
                var index = ConditionalFormatting.Rules.Count;
                foreach (var cf in slws.ConditionalFormattings)
                    foreach (var cfr in cf.Rules)
                        cfr.Priority += index;

                foreach (var cf2010 in slws.ConditionalFormattings2010)
                    foreach (var cfr2010 in cf2010.Rules)
                        if (cfr2010.Priority != null) cfr2010.Priority += index;

                var bIs2010 = false;
                var bIsDataBar2010 = false;
                var bIsIconSet2010 = false;
                var sGuid = string.Empty;
                var cf2010new = new SLConditionalFormatting2010();
                foreach (var pt in ConditionalFormatting.SequenceOfReferences)
                    cf2010new.ReferenceSequence.Add(new SLCellPointRange(pt.StartRowIndex, pt.StartColumnIndex,
                        pt.EndRowIndex, pt.EndColumnIndex));
                ...
                for (i = ConditionalFormatting.Rules.Count - 1; i >= 0; --i)
                {
                    ...
                    if (ConditionalFormatting.Rules[i].HasDifferentialFormat)
                        ConditionalFormatting.Rules[i].FormatId =
                            (uint)
                            SaveToStylesheetDifferentialFormat(
                                ConditionalFormatting.Rules[i].DifferentialFormat.ToHash());

                    bIsDataBar2010 = ConditionalFormatting.Rules[i].HasDataBar &&
                                     ConditionalFormatting.Rules[i].DataBar.Is2010;
                    bIsIconSet2010 = ConditionalFormatting.Rules[i].HasIconSet &&
                                     ConditionalFormatting.Rules[i].IconSet.Is2010;
        ...
        {
            var listExisting = new List<SLCellPointRange>();
            foreach (var dv in slws.DataValidations)
            ...
            var pt = DataValidation.SequenceOfReferences[0];
            ...
            for (i = 0; i < listExisting.Count; ++i)
                if (
                    !((pt.EndRowIndex < listExisting[i].StartRowIndex) ||
                      (pt.StartRowIndex > listExisting[i].EndRowIndex)
                      || (pt.EndColumnIndex < listExisting[i].StartColumnIndex) ||
                      (pt.StartColumnIndex > listExisting[i].EndColumnIndex)))
                {
                    result = false;
                    break;
                }
        ...
        /// <returns>
        ///     True if the table is successfully inserted. False otherwise. If it failed, check if the given table overlaps
        ///     any existing tables or merged cell range.
        /// </returns>
            ...
            if (result)
            {
                ...
                for (j = 0; j < Table.TableColumns.Count; ++j)
                {
                    ...
                    if ((tc.TotalsRowLabel != null) && (tc.TotalsRowLabel.Length > 0))
                    {
                        ...
                        c.NumericValue = DirectSaveToSharedStringTable(SLTool.XmlWrite(tc.TotalsRowLabel));
                    ...
                    if (tc.HasTotalsRowFunction)
                    {
                        ...
                        c.CellFormula.FormulaText = string.Format("SUBTOTAL({0},[{1}])",
                            GetFunctionNumber(tc.TotalsRowFunction), tc.Name);
                        if (!Calculate(tc.TotalsRowFunction, cells, out sResultText))
                        ...
                        if (slws.RowProperties.ContainsKey(pt.RowIndex))
                            iStyleIndex = slws.RowProperties[pt.RowIndex].StyleIndex;
                        if ((iStyleIndex == 0) && slws.ColumnProperties.ContainsKey(pt.ColumnIndex))
                            iStyleIndex = slws.ColumnProperties[pt.ColumnIndex].StyleIndex;
                        if (iStyleIndex != 0) c.StyleIndex = iStyleIndex;
        ...
        public SLPageSettings GetPageSettings(string SheetName)
        {
            ...
            var ps = new SLPageSettings(SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);

            var bSheetFound = false;
            var sheet = new SLSheet(string.Empty, 0, string.Empty, SLSheetType.Unknown);
            foreach (var s in slwb.Sheets)
            ...
            if (bSheetFound)
                        ...
                        while (oxr.Read())
                            ...
                            else if (oxr.ElementType == typeof(SheetView))
                            {
                                ...
                                if (!bFound)
                                {
                                    ...
                                    if (slsv.ZoomScalePageLayoutView != 0)
                                        ps.ZoomScalePageLayoutView = slsv.ZoomScalePageLayoutView;
                ...
                {
                    var csp = (ChartsheetPart) wbp.GetPartById(sheet.Id);
                ...
                {
                    var dsp = (DialogsheetPart) wbp.GetPartById(sheet.Id);
        ...
        internal void SetPageSettingsSheetView(SLPageSettings ps)
        {
            if (ps.bShowFormulas != null)
                ...
                {
                    var keys = slws.ColumnProperties.Keys.ToList();
                    SLColumnProperties cp;
                    ...
                    if (ps.bShowFormulas.Value)
                    {
                        ...
                        {
                            slws.SheetFormatProperties.DefaultColumnWidth = 2*
                                                                            slws.SheetFormatProperties
                                                                                .DefaultColumnWidth;
                    ...
                    else
                    {
                        ...
                        if (!slws.SheetFormatProperties.HasDefaultColumnWidth)
                        {
                            slws.SheetFormatProperties.DefaultColumnWidth = 0.5*
                                                                            slws.SheetFormatProperties
                                                                                .DefaultColumnWidth;
                            slws.SheetFormatProperties.HasDefaultColumnWidth = false;
                        }
                ...
                if (slws.SheetViews.Count > 0)
                {
                    ...
                    foreach (var sv in slws.SheetViews)
                        if (sv.WorkbookViewId == 0)
                        {
                            ...
                            if (ps.iZoomScalePageLayoutView != null)
                                sv.ZoomScalePageLayoutView = ps.iZoomScalePageLayoutView.Value;
                        }
        ...
        public void SetPageSettings(SLPageSettings PageSettings)
        {
            ...
            SetPageSettingsSheetView(PageSettings);
        }
            ...
            if (bSheetFound)
            {
                ...
                if (sheet.SheetType == SLSheetType.Worksheet)
                {
                    ...
                    if (PageSettings.HasSheetView)
                        ...
                        {
                            var bSheetViewFound = false;
                            foreach (SheetView sv in wsp.Worksheet.SheetViews)
                                if (sv.WorkbookViewId == 0)
                                {
                                    if (PageSettings.bShowFormulas != null)
                                        sv.ShowFormulas = PageSettings.bShowFormulas.Value;
                                    if (PageSettings.bShowGridLines != null)
                                        sv.ShowGridLines = PageSettings.bShowGridLines.Value;
                                    if (PageSettings.bShowRowColumnHeaders != null)
                                        sv.ShowRowColHeaders = PageSettings.bShowRowColumnHeaders.Value;
                                    ...
                                    if (PageSettings.iZoomScaleNormal != null)
                                        sv.ZoomScaleNormal = PageSettings.iZoomScaleNormal.Value;
                                    if (PageSettings.iZoomScalePageLayoutView != null)
                                        sv.ZoomScalePageLayoutView = PageSettings.iZoomScalePageLayoutView.Value;
                                }
        ...
        public void InsertPageBreak(int RowIndex, int ColumnIndex)
        {
            ...
            var iRowIndex = RowIndex - 1;
            var iColumnIndex = ColumnIndex - 1;
            ...
            if (iColumnIndex > 0)
            {
                ...
                b.Max = SLConstants.ColumnLimit - 1;
                b.ManualPageBreak = true;
        ...
        /// <summary>
        ///     Protect the currently selected worksheet. If the worksheet has protection (but not password protected), the current
        ///     protection options will be overwritten.
        /// </summary>
        /// <param name="ProtectOptions">An SLSheetProtection object with relevant options set.</param>
        /// <returns>
        ///     True if operation is successful. False otherwise. Note that if the worksheet already has password protection,
        ...
        /// </summary>
        /// <returns>
        ///     True if operation is successful. False otherwise. Note that if the worksheet is password protected or if the
        ///     worksheet has no sheet protection in the first place, false is also returned.
        /// </returns>
