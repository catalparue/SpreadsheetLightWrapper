...
namespace SpreadsheetLightWrapper.Core
{
    public partial class SLDocument
    {
        ...
        internal void CleanUpReallyEmptyCells()
        {
            ...
            var cellkeys = slws.Cells.Keys.ToList();
            foreach (var pt in cellkeys)
                if (slws.Cells[pt].IsEmpty)
                    slws.Cells.Remove(pt);
        }

        /// <summary>
        ///     Get existing cells in the currently selected worksheet. WARNING: This is only a snapshot. Any changes made to the
        ///     returned result are not used.
        /// </summary>
        /// <returns>A Dictionary of existing cells.</returns>
        public Dictionary<SLCellPoint, SLCell> GetCells()
        {
            var result = new Dictionary<SLCellPoint, SLCell>();

            var cellref = slws.Cells.Keys.ToList();
            foreach (var pt in cellref)
        ...
        /// <summary>
        ///     Set the cell value given a cell reference and a numeric value in string form. Use this when the source data is
        ///     numeric and is already in string form and parsing the data into numeric form is undesirable. Note that the numeric
        ///     string must be in invariant-culture mode, so "123456.789" is the accepted form even if the current culture displays
        ///     that as "123456,789".
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        /// <param name="Data">The cell value data.</param>
        ...
        /// <summary>
        ///     Set the cell value given the row index and column index and a numeric value in string form. Use this when the
        ///     source data is numeric and is already in string form and parsing the data into numeric form is undesirable. Note
        ///     that the numeric string must be in invariant-culture mode, so "123456.789" is the accepted form even if the current
        ///     culture displays that as "123456,789".
        /// </summary>
        /// <param name="RowIndex">The row index.</param>
        /// <param name="ColumnIndex">The column index.</param>
        /// <param name="Data">The cell value data.</param>
        ...
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        /// <param name="Data">The cell value data.</param>
        /// <param name="For1904Epoch">
        ///     True if using 1 Jan 1904 as the date epoch. False if using 1 Jan 1900 as the date epoch.
        ///     This is independent of the workbook's Date1904 property.
        /// </param>
        /// <returns>False if the cell reference is invalid. True otherwise.</returns>
        ...
        /// <param name="Data">The cell value data.</param>
        /// <param name="Format">
        ///     The format string used if the given date is before the date epoch. A date before the date epoch is
        ///     stored as a string, so the date precision is only as good as the format string. For example, "dd/MM/yyyy HH:mm:ss"
        ///     is more precise than "dd/MM/yyyy" because the latter loses information about the hours, minutes and seconds.
        /// </param>
        /// <returns>False if the cell reference is invalid. True otherwise.</returns>
        ...
        public bool SetCellValue(int RowIndex, int ColumnIndex, DateTime Data, string Format, bool For1904Epoch)
        {
            ...
            var fDateTime = SLTool.CalculateDaysFromEpoch(Data, For1904Epoch);
            // see CalculateDaysFromEpoch to see why there's a difference
            var fDateCheck = For1904Epoch ? 0.0 : 1.0;
            ...
            if (fDateTime < fDateCheck)
            {
                ...
                c.NumericValue = DirectSaveToSharedStringTable(Data.ToString(Format));
                slws.Cells[pt] = c;
            }
            else
            {
        ...
        private bool SetCellValueNumberFinal(string CellReference, bool IsNumeric, double NumericValue,
            string NumberData)
        {
            var iRowIndex = -1;
            var iColumnIndex = -1;
        ...
        private bool SetCellValueNumberFinal(int RowIndex, int ColumnIndex, bool IsNumeric, double NumericValue,
            string NumberData)
        {
            if (!SLTool.CheckRowColumnIndexLimit(RowIndex, ColumnIndex))
                return false;
        ...
        public bool SetCellValue(int RowIndex, int ColumnIndex, InlineString Data)
        {
            ...
            c.DataType = CellValues.SharedString;
            c.NumericValue = DirectSaveToSharedStringTable(Data);
            slws.Cells[pt] = c;
        ...
        public bool SetCellValue(int RowIndex, int ColumnIndex, string Data)
        {
            ...
            else if (Data.StartsWith("="))
            {
                ...
                {
                    c.DataType = CellValues.SharedString;
                    c.NumericValue = DirectSaveToSharedStringTable("=");
                    slws.Cells[pt] = c;
                }
                else
                {
            ...
            {
                c.DataType = CellValues.SharedString;
                c.NumericValue = DirectSaveToSharedStringTable(SLTool.XmlWrite(Data.Substring(1)));
                slws.Cells[pt] = c;
            }
            else
            {
                c.DataType = CellValues.SharedString;
                c.NumericValue = DirectSaveToSharedStringTable(SLTool.XmlWrite(Data));
                slws.Cells[pt] = c;
            }
        ...
        /// <summary>
        ///     Get the cell value as a boolean. If the cell value wasn't originally a boolean value, the return value is
        ///     undetermined (but is by default false).
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        ...
        /// <summary>
        ///     Get the cell value as a 32-bit integer. If the cell value wasn't originally an integer, the return value is
        ///     undetermined (but is by default 0).
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        /// <returns>A 32-bit integer cell value.</returns>
        public int GetCellValueAsInt32(string CellReference)
        {
            var iRowIndex = -1;
            var iColumnIndex = -1;
            if (!SLTool.FormatCellReferenceToRowColumnIndex(CellReference, out iRowIndex, out iColumnIndex))
        ...
        /// <param name="ColumnIndex">The column index.</param>
        /// <returns>A 32-bit integer cell value.</returns>
        public int GetCellValueAsInt32(int RowIndex, int ColumnIndex)
        {
            var result = 0;
            ...
            if (SLTool.CheckRowColumnIndexLimit(RowIndex, ColumnIndex))
            {
                ...
                if (slws.Cells.ContainsKey(pt))
                {
                    ...
                    if (c.DataType == CellValues.Number)
                        if (c.CellText != null)
                            int.TryParse(c.CellText, NumberStyles.Any, CultureInfo.InvariantCulture, out result);
                        else
        ...
        /// <summary>
        ///     Get the cell value as an unsigned 32-bit integer. If the cell value wasn't originally an integer, the return value
        ///     is undetermined (but is by default 0).
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        /// <returns>An unsigned 32-bit integer cell value.</returns>
        public uint GetCellValueAsUInt32(string CellReference)
        {
            var iRowIndex = -1;
            var iColumnIndex = -1;
            if (!SLTool.FormatCellReferenceToRowColumnIndex(CellReference, out iRowIndex, out iColumnIndex))
        ...
        /// <param name="ColumnIndex">The column index.</param>
        /// <returns>An unsigned 32-bit integer cell value.</returns>
        public uint GetCellValueAsUInt32(int RowIndex, int ColumnIndex)
        {
            uint result = 0;
            ...
            if (SLTool.CheckRowColumnIndexLimit(RowIndex, ColumnIndex))
            {
                ...
                if (slws.Cells.ContainsKey(pt))
                {
                    ...
                    if (c.DataType == CellValues.Number)
                        if (c.CellText != null)
                            uint.TryParse(c.CellText, NumberStyles.Any, CultureInfo.InvariantCulture, out result);
                        else
        ...
        /// <summary>
        ///     Get the cell value as a 64-bit integer. If the cell value wasn't originally an integer, the return value is
        ///     undetermined (but is by default 0).
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        /// <returns>A 64-bit integer cell value.</returns>
        public long GetCellValueAsInt64(string CellReference)
        {
            var iRowIndex = -1;
            var iColumnIndex = -1;
            if (!SLTool.FormatCellReferenceToRowColumnIndex(CellReference, out iRowIndex, out iColumnIndex))
        ...
        /// <param name="ColumnIndex">The column index.</param>
        /// <returns>A 64-bit integer cell value.</returns>
        public long GetCellValueAsInt64(int RowIndex, int ColumnIndex)
        {
            long result = 0;
            ...
            if (SLTool.CheckRowColumnIndexLimit(RowIndex, ColumnIndex))
            {
                ...
                if (slws.Cells.ContainsKey(pt))
                {
                    ...
                    if (c.DataType == CellValues.Number)
                        if (c.CellText != null)
                            long.TryParse(c.CellText, NumberStyles.Any, CultureInfo.InvariantCulture, out result);
                        else
        ...
        /// <summary>
        ///     Get the cell value as an unsigned 64-bit integer. If the cell value wasn't originally an integer, the return value
        ///     is undetermined (but is by default 0).
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        /// <returns>An unsigned 64-bit integer cell value.</returns>
        public ulong GetCellValueAsUInt64(string CellReference)
        {
            var iRowIndex = -1;
            var iColumnIndex = -1;
            if (!SLTool.FormatCellReferenceToRowColumnIndex(CellReference, out iRowIndex, out iColumnIndex))
        ...
        /// <param name="ColumnIndex">The column index.</param>
        /// <returns>An unsigned 64-bit integer cell value.</returns>
        public ulong GetCellValueAsUInt64(int RowIndex, int ColumnIndex)
        {
            ulong result = 0;
            ...
            if (SLTool.CheckRowColumnIndexLimit(RowIndex, ColumnIndex))
            {
                ...
                if (slws.Cells.ContainsKey(pt))
                {
                    ...
                    if (c.DataType == CellValues.Number)
                        if (c.CellText != null)
                            ulong.TryParse(c.CellText, NumberStyles.Any, CultureInfo.InvariantCulture, out result);
                        else
        ...
        /// <summary>
        ///     Get the cell value as a double precision floating point number. If the cell value wasn't originally a floating
        ///     point number, the return value is undetermined (but is by default 0).
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        ...
        /// <summary>
        ///     Get the cell value as a System.Decimal value. If the cell value wasn't originally an integer or floating point
        ///     number, the return value is undetermined (but is by default 0).
        /// </summary>
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        ...
        /// <summary>
        ///     Get the cell value as a System.DateTime value. If the cell value wasn't originally a date/time value, the return
        ...
        /// <param name="CellReference">The cell reference, such as "A1".</param>
        /// <param name="Format">
        ///     The format string used to parse the date value in the cell if the date is before the date epoch. A
        ///     date before the date epoch is stored as a string, so the date precision is only as good as the format string. For
        ///     example, "dd/MM/yyyy HH:mm:ss" is more precise than "dd/MM/yyyy" because the latter loses information about the
            ...
            if (SLTool.CheckRowColumnIndexLimit(RowIndex, ColumnIndex))
            {
                ...
                if (slws.Cells.ContainsKey(pt))
                {
                    ...
                    if (c.DataType == CellValues.Number)
                    {
                        if (c.CellText != null)
                        {
                            if (double.TryParse(c.CellText, NumberStyles.Any, CultureInfo.InvariantCulture,
                                out fDateOffset))
                    ...
                    {
                        var rst = new SLRstType(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont,
                            SimpleTheme.listThemeColors, SimpleTheme.listIndexedColors);
                        var index = 0;
                        try
                        {
                            if (c.CellText != null)
        ...
        public string GetCellValueAsString(int RowIndex, int ColumnIndex)
        {
            ...
            var index = 0;
            var rst = new SLRstType(SimpleTheme.MajorLatinFont, SimpleTheme.MinorLatinFont, SimpleTheme.listThemeColors,
        ...
        public bool SetActiveCell(string CellReference)
        {
            ...
            return SetActiveCell(iRowIndex, iColumnIndex);
        }
        ...
        {
            if ((RowIndex < 1) || (RowIndex > SLConstants.RowLimit) || (ColumnIndex < 1) ||
                (ColumnIndex > SLConstants.ColumnLimit))
                return false;
            ...
            {
                var bFound = false;
                var vActivePane = PaneValues.TopLeft;
                    ...
                    if (slws.SheetViews[i].WorkbookViewId == 0)
                    {
                        ...
                        if (slws.SheetViews[i].Selections.Count == 0)
                        {
                            if ((RowIndex != 1) || (ColumnIndex != 1))
                            {
                                sel = new SLSelection();
                                sel.ActiveCell = SLTool.ToCellReference(RowIndex, ColumnIndex);
                                sel.SequenceOfReferences.Add(new SLCellPointRange(RowIndex, ColumnIndex, RowIndex,
                                    ColumnIndex));
                        ...
                        else
                        {
                            ...
                            if (slws.SheetViews[i].HasPane)
                            {
                                ...
                                for (j = slws.SheetViews[i].Selections.Count - 1; j >= 0; --j)
                                    ...
                                    {
                                        slws.SheetViews[i].Selections[j].ActiveCell = SLTool.ToCellReference(RowIndex,
                                            ColumnIndex);
                                        slws.SheetViews[i].Selections[j].SequenceOfReferences.Clear();
                                        slws.SheetViews[i].Selections[j].SequenceOfReferences.Add(
                                            new SLCellPointRange(RowIndex, ColumnIndex, RowIndex, ColumnIndex));
                                    }
                            }
                            else
                            {
                                for (j = slws.SheetViews[i].Selections.Count - 1; j >= 0; --j)
                                    if (slws.SheetViews[i].Selections[j].Pane == PaneValues.TopLeft)
                                        ...
                                        {
                                            slws.SheetViews[i].Selections[j].ActiveCell =
                                                SLTool.ToCellReference(RowIndex, ColumnIndex);
                                            slws.SheetViews[i].Selections[j].SequenceOfReferences.Clear();
                                            slws.SheetViews[i].Selections[j].SequenceOfReferences.Add(
                                                new SLCellPointRange(RowIndex, ColumnIndex, RowIndex, ColumnIndex));
                                        }
        ...
        /// <summary>
        ///     Merge cells given a corner cell of the to-be-merged rectangle of cells, and the opposite corner cell. For example,
        ///     the top-left corner cell and the bottom-right corner cell. Or the bottom-left corner cell and the top-right corner
        ///     cell. No merging is done if it's just one cell.
        /// </summary>
        ...
        public bool MergeWorksheetCells(int StartRowIndex, int StartColumnIndex, int EndRowIndex, int EndColumnIndex)
        {
            ...
            if (SLTool.CheckRowColumnIndexLimit(iStartRowIndex, iStartColumnIndex) &&
                SLTool.CheckRowColumnIndexLimit(iEndRowIndex, iEndColumnIndex))
            {
                ...
                for (i = 0; i < slws.MergeCells.Count; ++i)
                {
                    ...
                    if (
                        !((iEndRowIndex < mc.StartRowIndex) || (iStartRowIndex > mc.EndRowIndex) ||
                          (iEndColumnIndex < mc.StartColumnIndex) || (iStartColumnIndex > mc.EndColumnIndex)))
                    {
                        result = false;
                        break;
                    }
                ...
                if (result)
                {
                    ...
                    for (i = 0; i < slws.Tables.Count; ++i)
                    {
                        t = slws.Tables[i];
                        if (
                            !((iEndRowIndex < t.StartRowIndex) || (iStartRowIndex > t.EndRowIndex) ||
                              (iEndColumnIndex < t.StartColumnIndex) || (iStartColumnIndex > t.EndColumnIndex)))
                        {
                            result = false;
                            break;
                        }
        ...
        /// <summary>
        ///     Unmerge cells given a corner cell of an existing merged rectangle of cells, and the opposite corner cell. For
        ...
        public bool UnmergeWorksheetCells(int StartRowIndex, int StartColumnIndex, int EndRowIndex, int EndColumnIndex)
        {
            ...
            var result = false;
            var mc = new SLMergeCell();
            for (var i = 0; i < slws.MergeCells.Count; ++i)
            {
                mc = slws.MergeCells[i];
                if ((mc.StartRowIndex == iStartRowIndex) && (mc.StartColumnIndex == iStartColumnIndex) &&
                    (mc.EndRowIndex == iEndRowIndex) && (mc.EndColumnIndex == iEndColumnIndex))
                {
        ...
        {
            var list = new List<SLMergeCell>();
            foreach (var mc in slws.MergeCells)
                list.Add(mc.Clone());
        ...
        public bool Filter(string StartCellReference, string EndCellReference)
        {
            ...
            return Filter(iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex);
        }
        ...
        public bool CopyCell(string CellReference, string AnchorCellReference)
        {
            var iRowIndex = -1;
            var iColumnIndex = -1;
            var iAnchorRowIndex = -1;
            var iAnchorColumnIndex = -1;
            if (!SLTool.FormatCellReferenceToRowColumnIndex(CellReference, out iRowIndex, out iColumnIndex)
                ||
                !SLTool.FormatCellReferenceToRowColumnIndex(AnchorCellReference, out iAnchorRowIndex,
                    out iAnchorColumnIndex))
                return false;
        ...
        public bool CopyCell(string StartCellReference, string EndCellReference, string AnchorCellReference)
        {
            ...
            return CopyCell(iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex, iAnchorRowIndex,
                iAnchorColumnIndex, false, SLPasteTypeValues.Paste);
        }
        ...
        public bool CopyCell(string StartCellReference, string EndCellReference, string AnchorCellReference, bool ToCut)
        {
            ...
            return CopyCell(iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex, iAnchorRowIndex,
                iAnchorColumnIndex, ToCut, SLPasteTypeValues.Paste);
        }
            ...
            SLPasteTypeValues PasteOption)
        {
            ...
            return CopyCell(iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex, iAnchorRowIndex,
                iAnchorColumnIndex, false, PasteOption);
        }
        ...
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCell(int StartRowIndex, int StartColumnIndex, int EndRowIndex, int EndColumnIndex,
            int AnchorRowIndex, int AnchorColumnIndex)
        {
            return CopyCell(StartRowIndex, StartColumnIndex, EndRowIndex, EndColumnIndex, AnchorRowIndex,
                AnchorColumnIndex, false, SLPasteTypeValues.Paste);
        }
        ...
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCell(int StartRowIndex, int StartColumnIndex, int EndRowIndex, int EndColumnIndex,
            int AnchorRowIndex, int AnchorColumnIndex, bool ToCut)
        {
            return CopyCell(StartRowIndex, StartColumnIndex, EndRowIndex, EndColumnIndex, AnchorRowIndex,
                AnchorColumnIndex, ToCut, SLPasteTypeValues.Paste);
        }
        ...
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCell(int StartRowIndex, int StartColumnIndex, int EndRowIndex, int EndColumnIndex,
            int AnchorRowIndex, int AnchorColumnIndex, SLPasteTypeValues PasteOption)
        {
            return CopyCell(StartRowIndex, StartColumnIndex, EndRowIndex, EndColumnIndex, AnchorRowIndex,
                AnchorColumnIndex, false, PasteOption);
        }

        private bool CopyCell(int StartRowIndex, int StartColumnIndex, int EndRowIndex, int EndColumnIndex,
            int AnchorRowIndex, int AnchorColumnIndex, bool ToCut, SLPasteTypeValues PasteOption)
        {
            int iStartRowIndex = 1, iEndRowIndex = 1, iStartColumnIndex = 1, iEndColumnIndex = 1;
            if (StartRowIndex < EndRowIndex)
            {
                ...
                && ((iStartRowIndex != AnchorRowIndex) || (iStartColumnIndex != AnchorColumnIndex)))
            {
                ...
                var rowdiff = AnchorRowIndex - iStartRowIndex;
                var coldiff = AnchorColumnIndex - iStartColumnIndex;
                var cells = new Dictionary<SLCellPoint, SLCell>();

                var colstyleindex = new Dictionary<int, uint>();
                var rowstyleindex = new Dictionary<int, uint>();

                var rowindexkeys = slws.RowProperties.Keys.ToList();
                SLRowProperties rp;
                foreach (var rowindex in rowindexkeys)
                {
                    rp = slws.RowProperties[rowindex];
                    rowstyleindex[rowindex] = rp.StyleIndex;
                }

                var colindexkeys = slws.ColumnProperties.Keys.ToList();
                SLColumnProperties cp;
                foreach (var colindex in colindexkeys)
                {
                            ...
                            switch (PasteOption)
                            {
                                case SLPasteTypeValues.Formatting:
                                    if (slws.Cells.ContainsKey(pt))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                iStyleIndexNew = 0;
                                                if (rowstyleindex.ContainsKey(newpt.RowIndex))
                                                    iStyleIndexNew = (int) rowstyleindex[newpt.RowIndex];
                                                if ((iStyleIndexNew == 0) &&
                                                    colstyleindex.ContainsKey(newpt.ColumnIndex))
                                                    iStyleIndexNew = (int) colstyleindex[newpt.ColumnIndex];
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            iStyleIndex = 0;
                                            if (rowstyleindex.ContainsKey(pt.RowIndex))
                                                iStyleIndex = (int) rowstyleindex[pt.RowIndex];
                                            if ((iStyleIndex == 0) && colstyleindex.ContainsKey(pt.ColumnIndex))
                                                iStyleIndex = (int) colstyleindex[pt.ColumnIndex];
                                        ...
                                        else
                                        {
                                            ...
                                            if (rowstyleindex.ContainsKey(newpt.RowIndex))
                                                iStyleIndexNew = (int) rowstyleindex[newpt.RowIndex];
                                            if ((iStyleIndexNew == 0) && colstyleindex.ContainsKey(newpt.ColumnIndex))
                                                iStyleIndexNew = (int) colstyleindex[newpt.ColumnIndex];
                                ...
                                case SLPasteTypeValues.Formulas:
                                    if (slws.Cells.ContainsKey(pt))
                                    {
                                        ...
                                        {
                                            newcell = slws.Cells[newpt].Clone();
                                            if (origcell.CellFormula != null)
                                                newcell.CellFormula = origcell.CellFormula.Clone();
                                ...
                                case SLPasteTypeValues.Transpose:
                                    ...
                                    if ((newpt.RowIndex <= SLConstants.RowLimit) &&
                                        (newpt.ColumnIndex <= SLConstants.ColumnLimit))
                                        if (slws.Cells.ContainsKey(pt))
                                        {
                                            origcell = slws.Cells[pt].Clone();
                                            cells[newpt] = origcell.Clone();
                ...
                var AnchorEndRowIndex = AnchorRowIndex + iEndRowIndex - iStartRowIndex;
                var AnchorEndColumnIndex = AnchorColumnIndex + iEndColumnIndex - iStartColumnIndex;
                ...
                var iNumberOfRows = iEndRowIndex - iStartRowIndex + 1;
                if (AnchorRowIndex <= iStartRowIndex) iNumberOfRows = -iNumberOfRows;
                var iNumberOfColumns = iEndColumnIndex - iStartColumnIndex + 1;
                ...
                foreach (var cellkey in cells.Keys)
                {
                    ...
                    if (PasteOption != SLPasteTypeValues.Transpose)
                        ProcessCellFormulaDelta(ref origcell, AnchorRowIndex, iNumberOfRows, AnchorColumnIndex,
                            iNumberOfColumns);
                    else
                        ProcessCellFormulaDelta(ref origcell, AnchorRowIndex, iNumberOfColumns, AnchorColumnIndex,
                            iNumberOfRows);
                ...
                var mca = GetWorksheetMergeCells();
                foreach (var mc in mca)
                        ...
                        && (mc.StartColumnIndex >= iStartColumnIndex) && (mc.EndColumnIndex <= iEndColumnIndex))
                    {
                        ...
                        if (PasteOption == SLPasteTypeValues.Transpose)
                        {
                            pt = new SLCellPoint(mc.StartRowIndex - iStartRowIndex,
                                mc.StartColumnIndex - iStartColumnIndex);
                            iSwap = pt.RowIndex;
                            ...
                            newpt = new SLCellPoint(mc.EndRowIndex - iStartRowIndex,
                                mc.EndColumnIndex - iStartColumnIndex);
                            iSwap = newpt.RowIndex;
                            newpt.RowIndex = newpt.ColumnIndex;
                            newpt.ColumnIndex = iSwap;
                            newpt.RowIndex = newpt.RowIndex + iStartRowIndex + rowdiff;
                            ...
                            MergeWorksheetCells(pt.RowIndex, pt.ColumnIndex, newpt.RowIndex, newpt.ColumnIndex);
                        }
                        else
                        {
                            MergeWorksheetCells(mc.StartRowIndex + rowdiff, mc.StartColumnIndex + coldiff,
                                mc.EndRowIndex + rowdiff, mc.EndColumnIndex + coldiff);
                        }
                    }
                ...
                if (slws.Hyperlinks.Count > 0)
                    if (ToCut)
                    {
                        foreach (var hl in slws.Hyperlinks)
                    ...
                    else
                    {
                        ...
                        {
                            var copiedhyperlinks = new List<SLHyperlink>();
                            SLHyperlink hlCopied;

                            // hyperlink ID, URL
                            var hlurl = new Dictionary<string, string>();
                            ...
                            if (!string.IsNullOrEmpty(gsSelectedWorksheetRelationshipID))
                            {
                                ...
                                foreach (var hlrel in wsp.HyperlinkRelationships)
                                    if (hlrel.IsExternal)
                                        hlurl[hlrel.Id] = hlrel.Uri.OriginalString;
                            }

                            var iOverlapStartRowIndex = 1;
                            var iOverlapStartColumnIndex = 1;
                            var iOverlapEndRowIndex = 1;
                            var iOverlapEndColumnIndex = 1;
                            foreach (var hl in slws.Hyperlinks)
                                      ...
                                      || (iStartColumnIndex > hl.Reference.EndColumnIndex)))
                                {
                                    ...
                                    hlCopied.Reference = new SLCellPointRange(iOverlapStartRowIndex,
                                        iOverlapStartColumnIndex, iOverlapEndRowIndex, iOverlapEndColumnIndex);
        ...
        public bool CopyCellFromWorksheet(string WorksheetName, string CellReference, string AnchorCellReference)
        {
            ...
            return CopyCellFromWorksheet(WorksheetName, iRowIndex, iColumnIndex, iRowIndex, iColumnIndex,
                iAnchorRowIndex, iAnchorColumnIndex, SLPasteTypeValues.Paste);
        }
            ...
            SLPasteTypeValues PasteOption)
        {
            ...
            return CopyCellFromWorksheet(WorksheetName, iRowIndex, iColumnIndex, iRowIndex, iColumnIndex,
                iAnchorRowIndex, iAnchorColumnIndex, PasteOption);
        }

        /// <summary>
        ///     Copy a range of cells from another worksheet to the currently selected worksheet, given the anchor cell of the
        ///     destination range (top-left cell).
        /// </summary>
        /// <param name="WorksheetName">The name of the source worksheet.</param>
        /// <param name="StartCellReference">
        ///     The cell reference of the start cell of the cell range, such as "A1". This is
        ...
        /// <param name="AnchorCellReference">The cell reference of the anchor cell, such as "A1".</param>
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCellFromWorksheet(string WorksheetName, string StartCellReference, string EndCellReference,
            string AnchorCellReference)
        {
            var iStartRowIndex = -1;
            var iStartColumnIndex = -1;
            var iEndRowIndex = -1;
            ...
            return CopyCellFromWorksheet(WorksheetName, iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex,
                iAnchorRowIndex, iAnchorColumnIndex);
        }
        ...
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCellFromWorksheet(string WorksheetName, string StartCellReference, string EndCellReference,
            string AnchorCellReference, SLPasteTypeValues PasteOption)
        {
            var iStartRowIndex = -1;
            var iStartColumnIndex = -1;
            var iEndRowIndex = -1;
        ...
        /// <param name="AnchorColumnIndex">The column index of the anchor cell.</param>
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCellFromWorksheet(string WorksheetName, int RowIndex, int ColumnIndex, int AnchorRowIndex,
            int AnchorColumnIndex)
        {
            return CopyCellFromWorksheet(WorksheetName, RowIndex, ColumnIndex, RowIndex, ColumnIndex, AnchorRowIndex,
                AnchorColumnIndex);
        }
        ...
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCellFromWorksheet(string WorksheetName, int RowIndex, int ColumnIndex, int AnchorRowIndex,
            int AnchorColumnIndex, SLPasteTypeValues PasteOption)
        {
            return CopyCellFromWorksheet(WorksheetName, RowIndex, ColumnIndex, RowIndex, ColumnIndex, AnchorRowIndex,
                AnchorColumnIndex, PasteOption);
        }
        ...
        /// <param name="AnchorColumnIndex">The column index of the anchor cell.</param>
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCellFromWorksheet(string WorksheetName, int StartRowIndex, int StartColumnIndex, int EndRowIndex,
            int EndColumnIndex, int AnchorRowIndex, int AnchorColumnIndex)
        {
            return CopyCellFromWorksheet(WorksheetName, StartRowIndex, StartColumnIndex, EndRowIndex, EndColumnIndex,
                AnchorRowIndex, AnchorColumnIndex, SLPasteTypeValues.Paste);
        }
        ...
        /// <returns>True if successful. False otherwise.</returns>
        public bool CopyCellFromWorksheet(string WorksheetName, int StartRowIndex, int StartColumnIndex, int EndRowIndex,
            int EndColumnIndex, int AnchorRowIndex, int AnchorColumnIndex, SLPasteTypeValues PasteOption)
        {
            int iStartRowIndex = 1, iEndRowIndex = 1, iStartColumnIndex = 1, iEndColumnIndex = 1;
            if (StartRowIndex < EndRowIndex)
            {
            ...
            if (WorksheetName.Equals(gsSelectedWorksheetName, StringComparison.OrdinalIgnoreCase))
                return CopyCell(iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex, AnchorRowIndex,
                    AnchorColumnIndex, false);
                ...
                && (AnchorColumnIndex >= 1) && (AnchorColumnIndex <= SLConstants.ColumnLimit))
            {
                ...
                var wsp = (WorksheetPart) wbp.GetPartById(sRelId);
                ...
                var coldiff = AnchorColumnIndex - iStartColumnIndex;
                var cells = new Dictionary<SLCellPoint, SLCell>();
                var sourcecells = new Dictionary<SLCellPoint, SLCell>();

                var sourcecolstyleindex = new Dictionary<int, uint>();
                var sourcerowstyleindex = new Dictionary<int, uint>();

                var sCellRef = string.Empty;
                var hsCellRef = new HashSet<string>();
                ...
                // hyperlink ID, URL
                var hlurl = new Dictionary<string, string>();
                var sourcehyperlinks = new List<SLHyperlink>();
                        ...
                        else if (oxr.ElementType == typeof(Row))
                        {
                            ...
                            if (r.RowIndex != null)
                                if (r.StyleIndex != null)
                                    sourcerowstyleindex[(int) r.RowIndex.Value] = r.StyleIndex.Value;
                                else sourcerowstyleindex[(int) r.RowIndex.Value] = 0;

                            using (var oxrRow = OpenXmlReader.Create(r))
                            {
                                while (oxrRow.Read())
                        ...
                        switch (PasteOption)
                        {
                            case SLPasteTypeValues.Formatting:
                                ...
                                else
                                {
                                    ...
                                    {
                                        iStyleIndex = 0;
                                        if (sourcerowstyleindex.ContainsKey(pt.RowIndex))
                                            iStyleIndex = (int) sourcerowstyleindex[pt.RowIndex];
                                        if ((iStyleIndex == 0) && sourcecolstyleindex.ContainsKey(pt.ColumnIndex))
                                            iStyleIndex = (int) sourcecolstyleindex[pt.ColumnIndex];
                ...
                if (sourcehyperlinks.Count > 0)
                    if ((PasteOption == SLPasteTypeValues.Paste) || (PasteOption == SLPasteTypeValues.Transpose))
                    {
                        ...
                        var iOverlapEndRowIndex = 1;
                        var iOverlapEndColumnIndex = 1;
                        foreach (var hl in sourcehyperlinks)
                            if (!((iEndRowIndex < hl.Reference.StartRowIndex)
                                  || (iStartRowIndex > hl.Reference.EndRowIndex)
                                  || (iEndColumnIndex < hl.Reference.StartColumnIndex)
                                  || (iStartColumnIndex > hl.Reference.EndColumnIndex)))
        ...
        /// </summary>
        /// <returns>
        ///     True if content has been cleared. False otherwise. If there are no content in the worksheet, false is also
        ///     returned.
        /// </returns>
        public bool ClearCellContent()
        {
            var result = false;
            var list = slws.Cells.Keys.ToList();
            if (list.Count > 0) result = true;
            foreach (var pt in list)
                ClearCellContentData(pt);

            return result;
        }

        /// <summary>
        ///     Clear all cell content within specified rows and columns. If the top-left cell of a merged cell is within specified
        ///     rows and columns, the merged cell content is also cleared.
        /// </summary>
        /// <param name="StartCellReference">
        ///     The cell reference of the start cell of the cell range to be cleared, such as "A1".
        ///     This is typically the top-left cell.
        /// </param>
        /// <param name="EndCellReference">
        ///     The cell reference of the end cell of the cell range to be cleared, such as "A1". This
        ///     is typically the bottom-right cell.
        /// </param>
        /// <returns>
        ///     True if content has been cleared. False otherwise. If there are no content within specified rows and columns,
            ...
            else
            {
                ...
                foreach (var pt in list)
                    if ((iStartRowIndex <= pt.RowIndex) && (pt.RowIndex <= iEndRowIndex) &&
                        (iStartColumnIndex <= pt.ColumnIndex) && (pt.ColumnIndex <= iEndColumnIndex))
                    {
                        ClearCellContentData(pt);
                        result = true;
                    }
        ...
        private void ClearCellContentData(SLCellPoint pt)
        {
            ...
            if ((c.StyleIndex != 0) || (c.CellMetaIndex != 0) || (c.ValueMetaIndex != 0) || c.ShowPhonetic)
        ...
        internal void ProcessCellFormulaDelta(ref SLCell cell, int StartRowIndex, int RowDelta, int StartColumnIndex,
            int ColumnDelta)
        {
            if ((cell.CellText != null) && cell.CellText.StartsWith("="))
                cell.CellText = AddDeleteCellFormulaDelta(cell.CellText, StartRowIndex, RowDelta, StartColumnIndex,
                    ColumnDelta);
            if ((cell.CellFormula != null) && (cell.CellFormula.FormulaType == CellFormulaValues.Normal))
            {
                cell.CellFormula.FormulaText = AddDeleteCellFormulaDelta(cell.CellFormula.FormulaText, StartRowIndex,
                    RowDelta, StartColumnIndex, ColumnDelta);
        ...
        /// <param name="ColumnDelta"></param>
        /// <returns></returns>
        internal string AddDeleteCellFormulaDelta(string CellFormula, int StartRowIndex, int RowDelta,
            int StartColumnIndex, int ColumnDelta)
        {
            var result = string.Empty;
            var sToCheck = CellFormula;
            var sSheetNameRegex = string.Format("({0}!|'{0}'!)?", gsSelectedWorksheetName);
            ...
            var sCellRefRegex = @"(?<cellref>" + sSheetNameRegex + @"\$?[a-zA-Z]{1,3}\$?[0-9]{1,7}(\s*:\s*" +
                                sSheetNameRegex + @"\$?[a-zA-Z]{1,3}\$?[0-9]{1,7})?)";
            ...
            var sRegexCheck = @"(?<cellrefpre>^|[+\-*/^=<>,(]|\s)" + sCellRefRegex +
                              @"(?<cellrefpost>[+\-*/^=<>,)]|\s|$)";
            var index = 0;
            var iDoubleQuoteCount = 0;
            Match m;
            ...
            while (m.Success)
            {
                ...
                if (iDoubleQuoteCount%2 == 0)
                    result += AddDeleteCellReferenceDelta(m.Groups["cellref"].Value, StartRowIndex, RowDelta,
                        StartColumnIndex, ColumnDelta);
                else
        ...
        /// <returns></returns>
        internal string AddDeleteDefinedNameRowColumnRangeDelta(string DefinedNameValue, bool CheckForRow,
            int StartRange, int Delta)
        {
            var result = string.Empty;
            var sToCheck = DefinedNameValue;
            var sSheetNameRegex = string.Format("({0}!|'{0}'!)?", gsSelectedWorksheetName);
            ...
            if (CheckForRow)
                sCellRefRegex = @"(?<cellref>" + sSheetNameRegex + @"\$[0-9]{1,7}\s*:\s*" + sSheetNameRegex +
                                @"\$[0-9]{1,7})";
            else
                sCellRefRegex = @"(?<cellref>" + sSheetNameRegex + @"\$[a-zA-Z]{1,3}\s*:\s*" + sSheetNameRegex +
                                @"\$[a-zA-Z]{1,3})";
            // The only characters that can be before a valid cell reference are +-*/^=<>,( and the space.
            // The cell reference can also be at the start of the string, thus the ^
            // The only characters that can be after a valid cell reference are +-*/^=<>,) and the space.
        ...
        internal string AddDeleteRowColumnRangeDelta(string Range, bool CheckForRow, int StartRange, int Delta)
        {
            ...
            else
            {
                ...
                if (CheckForRow)
                {
                    ...
                    if ((iRowIndex < 1) || (iRowIndex > SLConstants.RowLimit) || (iRowIndex2 < 1) ||
                        (iRowIndex2 > SLConstants.RowLimit))
                        result = sSheetName + "#REF!";
                    else
                        result = string.Format("{0}${1}:{2}${3}", sSheetName,
                            iRowIndex.ToString(CultureInfo.InvariantCulture), sSheetName2,
                ...
                else
                {
                    ...
                    if ((iColumnIndex > 0) && (iColumnIndex2 > 0))
                        ...
                        else
                        {
                            ...
                            {
                                DeleteRowColumnIndexDelta(StartRange, iEndRange, -Delta, ref iColumnIndex,
                                    ref iColumnIndex2);
                            }
                        }

                    if ((iColumnIndex < 1) || (iColumnIndex > SLConstants.ColumnLimit) || (iColumnIndex2 < 1) ||
                        (iColumnIndex2 > SLConstants.ColumnLimit))
                        result = sSheetName + "#REF!";
                    else
                        result = string.Format("{0}${1}:{2}${3}", sSheetName, SLTool.ToColumnName(iColumnIndex),
                            sSheetName2, SLTool.ToColumnName(iColumnIndex2));
                }
            }
        ...
        /// <param name="ColumnDelta"></param>
        /// <returns></returns>
        internal string AddDeleteCellReferenceDelta(string CellReference, int StartRowIndex, int RowDelta,
            int StartColumnIndex, int ColumnDelta)
        {
            var result = string.Empty;
            string sSheetName = string.Empty, sSheetName2 = string.Empty;
            ...
            var bIsRowAbsolute = Regex.IsMatch(sCellRef, @"\$[0-9]{1,7}");
            var bIsColumnAbsolute = Regex.IsMatch(sCellRef, @"\$[a-zA-Z]{1,3}");
            ...
            if (!bIsRange)
            {
                if (SLTool.FormatCellReferenceToRowColumnIndex(sCellRef, out iRowIndex, out iColumnIndex))
                {
                    ...
                    if ((iRowIndex < 1) || (iRowIndex > SLConstants.RowLimit) || (iColumnIndex < 1) ||
                        (iColumnIndex > SLConstants.ColumnLimit))
                        result = "#REF!";
                    else
                        result = sSheetName + (bIsColumnAbsolute ? "$" : "") + SLTool.ToColumnName(iColumnIndex) +
                                 (bIsRowAbsolute ? "$" : "") + iRowIndex.ToString(CultureInfo.InvariantCulture);
                }
            }
            else
            {
                    ...
                    SLTool.FormatCellReferenceToRowColumnIndex(sCellRef2, out iRowIndex2, out iColumnIndex2))
                {
                    if (StartRowIndex > 0)
                        ...
                        else
                        {
                            ...
                            {
                                DeleteRowColumnIndexDelta(StartRowIndex, iEndRowIndex, -RowDelta, ref iRowIndex,
                                    ref iRowIndex2);
                            }
                        }
                    ...
                    if (StartColumnIndex > 0)
                        if (ColumnDelta > 0)
                        {
                            AddRowColumnIndexDelta(StartColumnIndex, ColumnDelta, false, ref iColumnIndex,
                                ref iColumnIndex2);
                        }
                        else
                        {
                            ...
                            {
                                DeleteRowColumnIndexDelta(StartColumnIndex, iEndColumnIndex, -ColumnDelta,
                                    ref iColumnIndex, ref iColumnIndex2);
                            }
                        }

                    if ((iRowIndex < 1) || (iRowIndex > SLConstants.RowLimit) || (iColumnIndex < 1) ||
                        (iColumnIndex > SLConstants.ColumnLimit) || (iRowIndex2 < 1) ||
                        (iRowIndex2 > SLConstants.RowLimit) ||
                    ...
                    else
                    {
                        ...
                        result = sSheetName + (bIsColumnAbsolute ? "$" : "") + SLTool.ToColumnName(iColumnIndex) +
                                 (bIsRowAbsolute ? "$" : "") + iRowIndex.ToString(CultureInfo.InvariantCulture);
                        result += ":" + sSheetName2 + (bIsColumnAbsolute2 ? "$" : "") +
                                  SLTool.ToColumnName(iColumnIndex2) + (bIsRowAbsolute2 ? "$" : "") +
                                  iRowIndex2.ToString(CultureInfo.InvariantCulture);
                    }
                }
            }
